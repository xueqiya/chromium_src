...
void StoreModuleExports_0(compiler::CodeAssemblerState* state_, TNode<Module> p_o, TNode<ObjectHashTable> p_v) {
  ...
  if (block0.is_used()) {
    ...
    CodeStubAssembler(state_).StoreReference<ObjectHashTable>(CodeStubAssembler::Reference{tmp0, tmp2}, tmp1);
...
void StoreJSModuleNamespaceModule_0(compiler::CodeAssemblerState* state_, TNode<JSModuleNamespace> p_o, TNode<Module> p_v) {
  ...
  if (block0.is_used()) {
    ...
    USE(tmp2);
    tmp2 = FromConstexpr_intptr_constexpr_int31_0(state_, 12);
    CodeStubAssembler(state_).StoreReference<Module>(CodeStubAssembler::Reference{tmp0, tmp2}, tmp1);
    ca_.Goto(&block2, tmp0, tmp1);
  }
