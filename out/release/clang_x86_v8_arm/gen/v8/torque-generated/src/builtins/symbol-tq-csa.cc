...
TNode<Symbol> ThisSymbolValue_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<Object> p_receiver, const char* p_method) {
  ...
  if (block0.is_used()) {
    ...
    TNode<Object> tmp2;
    USE(tmp2);
    tmp2 = CodeStubAssembler(state_).ToThisValue(TNode<Context>{tmp0}, TNode<Object>{tmp1}, PrimitiveType::kSymbol, p_method);
    TNode<Symbol> tmp3;
    USE(tmp3);
    tmp3 = UnsafeCast_Symbol_0(state_, TNode<Context>{tmp0}, TNode<Object>{tmp2});
...
USE(parameter1);
  ...
  if (block0.is_used()) {
    ...
    TNode<Symbol> tmp2;
    USE(tmp2);
    tmp2 = ThisSymbolValue_0(state_, TNode<Context>{tmp0}, TNode<Object>{tmp1}, "Symbol.prototype.description");
    TNode<IntPtrT> tmp3;
    USE(tmp3);
    tmp3 = FromConstexpr_intptr_constexpr_int31_0(state_, 12);
    TNode<PrimitiveHeapObject>tmp4 = CodeStubAssembler(state_).LoadReference<PrimitiveHeapObject>(CodeStubAssembler::Reference{tmp2, tmp3});
...
USE(parameter1);
  ...
  if (block0.is_used()) {
    ...
    TNode<Symbol> tmp3;
    USE(tmp3);
    tmp3 = ThisSymbolValue_0(state_, TNode<Context>{tmp0}, TNode<Object>{tmp1}, "Symbol.prototype [ @@toPrimitive ]");
...
USE(parameter1);
  ...
  if (block0.is_used()) {
    ...
    TNode<Symbol> tmp2;
    USE(tmp2);
    tmp2 = ThisSymbolValue_0(state_, TNode<Context>{tmp0}, TNode<Object>{tmp1}, "Symbol.prototype.toString");
    TNode<String> tmp3;
    tmp3 = TORQUE_CAST(CodeStubAssembler(state_).CallRuntime(Runtime::kSymbolDescriptiveString, tmp0, tmp2)); 
    USE(tmp3);
    CodeStubAssembler(state_).Return(tmp3);
  }
}
...
USE(parameter1);
  ...
  if (block0.is_used()) {
    ...
    TNode<Symbol> tmp2;
    USE(tmp2);
    tmp2 = ThisSymbolValue_0(state_, TNode<Context>{tmp0}, TNode<Object>{tmp1}, "Symbol.prototype.valueOf");
...
TNode<Symbol> UnsafeCast_Symbol_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<Object> p_o) {
  ...
  compiler::CodeAssemblerParameterizedLabel<Context, Object, Symbol> block6(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
