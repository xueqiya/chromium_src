...
USE(parameter1);
  ...
  if (block15.is_used()) {
    ...
    TNode<Object> tmp88;
    tmp88 = CodeStubAssembler(state_).CallBuiltin(Builtins::kArraySomeLoopContinuation, tmp75, tmp81, tmp82, tmp78, tmp86, tmp81, tmp83, tmp85, tmp87);
    USE(tmp88);
...
USE(parameter1);
  ...
  if (block15.is_used()) {
    ...
    ca_.Branch(tmp95, &block17, &block18, tmp83, tmp84, tmp85, tmp86, tmp87, tmp88, tmp89, tmp90, tmp91, tmp92, tmp94);
  }
  ...
  if (block17.is_used()) {
    ...
    TNode<Number> tmp105;
    TNode<Number> tmp106;
    ca_.Bind(&block17, &tmp96, &tmp97, &tmp98, &tmp99, &tmp100, &tmp101, &tmp102, &tmp103, &tmp104, &tmp105, &tmp106);
    TNode<Oddball> tmp107;
    USE(tmp107);
    tmp107 = True_0(state_);
  ...
  if (block18.is_used()) {
    ...
    TNode<Number> tmp117;
    TNode<Number> tmp118;
    ca_.Bind(&block18, &tmp108, &tmp109, &tmp110, &tmp111, &tmp112, &tmp113, &tmp114, &tmp115, &tmp116, &tmp117, &tmp118);
    TNode<Number> tmp119;
    USE(tmp119);
    tmp119 = FromConstexpr_Number_constexpr_int31_0(state_, 1);
    TNode<Number> tmp120;
    USE(tmp120);
    tmp120 = CodeStubAssembler(state_).NumberAdd(TNode<Number>{tmp117}, TNode<Number>{tmp119});
    ...
    TNode<Object> tmp123;
    tmp123 = CodeStubAssembler(state_).CallBuiltin(Builtins::kArraySomeLoopContinuation, tmp108, tmp115, tmp116, tmp111, tmp121, tmp115, tmp120, tmp118, tmp122);
    USE(tmp123);
...
TF_BUILTIN(ArraySomeLoopContinuation, CodeStubAssembler) {
  ...
  if (block6.is_used()) {
    ...
    TNode<Number> tmp85;
    USE(tmp85);
    tmp85 = FromConstexpr_Number_constexpr_int31_0(state_, 1);
    TNode<Number> tmp86;
    USE(tmp86);
    tmp86 = CodeStubAssembler(state_).NumberAdd(TNode<Number>{tmp83}, TNode<Number>{tmp85});
    ca_.Goto(&block3, tmp74, tmp75, tmp76, tmp77, tmp78, tmp79, tmp80, tmp81, tmp82, tmp86);
  }
  ...
  if (block2.is_used()) {
    ...
    TNode<Object> tmp95;
    TNode<Number> tmp96;
    ca_.Bind(&block2, &tmp87, &tmp88, &tmp89, &tmp90, &tmp91, &tmp92, &tmp93, &tmp94, &tmp95, &tmp96);
    TNode<Oddball> tmp97;
    USE(tmp97);
    tmp97 = False_0(state_);
...
TNode<Object> FastArraySome_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSReceiver> p_o, TNode<Number> p_len, TNode<JSReceiver> p_callbackfn, TNode<Object> p_thisArg, compiler::CodeAssemblerLabel* label_Bailout, compiler::TypedCodeAssemblerVariable<Smi>* label_Bailout_parameter_0) {
  ...
  if (block27.is_used()) {
    ...
    ca_.Branch(tmp326, &block36, &block37, tmp306, tmp307, tmp308, tmp309, tmp310, tmp311, tmp312, tmp313, tmp314, tmp315, tmp316, tmp317, tmp318, tmp319, tmp324, tmp325);
  }
  ...
  if (block26.is_used()) {
    ...
    TNode<BoolT> tmp339;
    TNode<BoolT> tmp340;
    ca_.Bind(&block26, &tmp327, &tmp328, &tmp329, &tmp330, &tmp331, &tmp332, &tmp333, &tmp334, &tmp335, &tmp336, &tmp337, &tmp338, &tmp339, &tmp340);
    ca_.Goto(&block14, tmp327, tmp328, tmp329, tmp330, tmp331, tmp332, tmp333, tmp334, tmp335, tmp336, tmp337, tmp338, tmp339, tmp340);
  }
  ...
  if (block36.is_used()) {
    ...
    TNode<Object> tmp355;
    TNode<Object> tmp356;
    ca_.Bind(&block36, &tmp341, &tmp342, &tmp343, &tmp344, &tmp345, &tmp346, &tmp347, &tmp348, &tmp349, &tmp350, &tmp351, &tmp352, &tmp353, &tmp354, &tmp355, &tmp356);
    TNode<Oddball> tmp357;
    USE(tmp357);
    tmp357 = True_0(state_);
    ca_.Goto(&block2, tmp341, tmp342, tmp343, tmp344, tmp345, tmp357);
  }
  ...
  if (block37.is_used()) {
    ...
    TNode<Object> tmp372;
    TNode<Object> tmp373;
    ca_.Bind(&block37, &tmp358, &tmp359, &tmp360, &tmp361, &tmp362, &tmp363, &tmp364, &tmp365, &tmp366, &tmp367, &tmp368, &tmp369, &tmp370, &tmp371, &tmp372, &tmp373);
    ca_.Goto(&block14, tmp358, tmp359, tmp360, tmp361, tmp362, tmp363, tmp364, tmp365, tmp366, tmp367, tmp368, tmp369, tmp370, tmp371);
  }
  ...
  if (block14.is_used()) {
    ...
    TNode<BoolT> tmp386;
    TNode<BoolT> tmp387;
    ca_.Bind(&block14, &tmp374, &tmp375, &tmp376, &tmp377, &tmp378, &tmp379, &tmp380, &tmp381, &tmp382, &tmp383, &tmp384, &tmp385, &tmp386, &tmp387);
    TNode<Smi> tmp388;
    USE(tmp388);
    tmp388 = FromConstexpr_Smi_constexpr_int31_0(state_, 1);
    TNode<Smi> tmp389;
    USE(tmp389);
    tmp389 = CodeStubAssembler(state_).SmiAdd(TNode<Smi>{tmp379}, TNode<Smi>{tmp388});
    ca_.Goto(&block13, tmp374, tmp375, tmp376, tmp377, tmp378, tmp389, tmp380, tmp381, tmp382, tmp383, tmp384, tmp385, tmp386, tmp387);
  }
  ...
  if (block12.is_used()) {
    ...
    TNode<BoolT> tmp402;
    TNode<BoolT> tmp403;
    ca_.Bind(&block12, &tmp390, &tmp391, &tmp392, &tmp393, &tmp394, &tmp395, &tmp396, &tmp397, &tmp398, &tmp399, &tmp400, &tmp401, &tmp402, &tmp403);
    TNode<Oddball> tmp404;
    USE(tmp404);
    tmp404 = False_0(state_);
    ca_.Goto(&block2, tmp390, tmp391, tmp392, tmp393, tmp394, tmp404);
  }
  ...
  if (block2.is_used()) {
    ...
    TNode<Object> tmp409;
    TNode<Object> tmp410;
    ca_.Bind(&block2, &tmp405, &tmp406, &tmp407, &tmp408, &tmp409, &tmp410);
    ca_.Goto(&block38, tmp405, tmp406, tmp407, tmp408, tmp409, tmp410);
  }

  if (block1.is_used()) {
    TNode<Smi> tmp411;
    ca_.Bind(&block1, &tmp411);
    *label_Bailout_parameter_0 = tmp411;
    ...
    TNode<Object> tmp416;
    TNode<Object> tmp417;
    ca_.Bind(&block38, &tmp412, &tmp413, &tmp414, &tmp415, &tmp416, &tmp417);
  return TNode<Object>{tmp417};
}
...
USE(parameter1);
  ...
  if (block0.is_used()) {
    ...
    TNode<Object> tmp5;
    USE(tmp5);
    tmp5 = RequireObjectCoercible_0(state_, TNode<Context>{tmp3}, TNode<Object>{tmp4}, "Array.prototype.some");
    TNode<JSReceiver> tmp6;
    USE(tmp6);
  ...
  if (block9.is_used()) {
    ...
    compiler::CodeAssemblerLabel label0(&ca_);
    tmp76 = FastArraySome_0(state_, TNode<Context>{tmp70}, TNode<JSReceiver>{tmp72}, TNode<Number>{tmp73}, TNode<JSReceiver>{tmp74}, TNode<Object>{tmp75}, &label0, &result_0_0);
  ...
  if (block14.is_used()) {
    ...
    TNode<Object> tmp94;
    tmp94 = CodeStubAssembler(state_).CallBuiltin(Builtins::kArraySomeLoopContinuation, tmp80, tmp82, tmp84, tmp85, tmp92, tmp82, tmp91, tmp83, tmp93);
    USE(tmp94);
