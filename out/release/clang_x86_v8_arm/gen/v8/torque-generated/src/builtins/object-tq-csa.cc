...
TF_BUILTIN(CreateObjectWithoutProperties, CodeStubAssembler) {
  ...
  compiler::CodeAssemblerParameterizedLabel<Context, Object, NativeContext, Map, FixedArray, Object, JSReceiver, JSFunction, JSReceiver, JSReceiver, IntPtrT, Map> block14(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  compiler::CodeAssemblerParameterizedLabel<Context, Object, NativeContext, Map, FixedArray, Object, JSReceiver, JSFunction, JSReceiver, JSReceiver, IntPtrT, Map, PrototypeInfo> block13(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  ...
  if (block5.is_used()) {
    ...
    TNode<IntPtrT> tmp22;
    USE(tmp22);
    tmp22 = FromConstexpr_NativeContextSlot_constexpr_SLOW_OBJECT_WITH_NULL_PROTOTYPE_MAP_0(state_, Context::Field::SLOW_OBJECT_WITH_NULL_PROTOTYPE_MAP);
    TNode<Object> tmp23;
    USE(tmp23);
  ...
  if (block9.is_used()) {
    ...
    TNode<IntPtrT> tmp44;
    USE(tmp44);
    tmp44 = FromConstexpr_NativeContextSlot_constexpr_OBJECT_FUNCTION_INDEX_0(state_, Context::Field::OBJECT_FUNCTION_INDEX);
    TNode<Object> tmp45;
    USE(tmp45);
  ...
  if (block11.is_used()) {
    ...
    ca_.Goto(&block13, tmp53, tmp54, tmp55, tmp56, tmp57, tmp58, tmp59, tmp60, tmp59, tmp59, tmp61, tmp62, tmp63);
    if (label0.is_used()) {
      ca_.Bind(&label0);
      ca_.Goto(&block14, tmp53, tmp54, tmp55, tmp56, tmp57, tmp58, tmp59, tmp60, tmp59, tmp59, tmp61, tmp62);
    }
  }
  ...
  if (block14.is_used()) {
    ...
    TNode<IntPtrT> tmp74;
    TNode<Map> tmp75;
    ca_.Bind(&block14, &tmp64, &tmp65, &tmp66, &tmp67, &tmp68, &tmp69, &tmp70, &tmp71, &tmp72, &tmp73, &tmp74, &tmp75);
  ...
  if (block13.is_used()) {
    ...
    TNode<IntPtrT> tmp86;
    TNode<Map> tmp87;
    TNode<PrototypeInfo> tmp88;
    ca_.Bind(&block13, &tmp76, &tmp77, &tmp78, &tmp79, &tmp80, &tmp81, &tmp82, &tmp83, &tmp84, &tmp85, &tmp86, &tmp87, &tmp88);
    TNode<IntPtrT> tmp89;
    USE(tmp89);
    tmp89 = FromConstexpr_intptr_constexpr_int31_0(state_, 24);
    TNode<MaybeObject>tmp90 = CodeStubAssembler(state_).LoadReference<MaybeObject>(CodeStubAssembler::Reference{tmp88, tmp89});
    TNode<Oddball> tmp91;
    USE(tmp91);
    compiler::CodeAssemblerLabel label0(&ca_);
    tmp91 = Cast_Undefined_0(state_, TNode<MaybeObject>{tmp90}, &label0);
    ca_.Goto(&block17, tmp76, tmp77, tmp78, tmp79, tmp80, tmp81, tmp82, tmp83, tmp88, tmp90, tmp90, tmp91);
    if (label0.is_used()) {
      ca_.Bind(&label0);
      ca_.Goto(&block18, tmp76, tmp77, tmp78, tmp79, tmp80, tmp81, tmp82, tmp83, tmp88, tmp90, tmp90);
    }
  }
  ...
  if (block18.is_used()) {
    ...
    TNode<PrototypeInfo> tmp100;
    TNode<MaybeObject> tmp101;
    TNode<MaybeObject> tmp102;
    ca_.Bind(&block18, &tmp92, &tmp93, &tmp94, &tmp95, &tmp96, &tmp97, &tmp98, &tmp99, &tmp100, &tmp101, &tmp102);
    TNode<Map> tmp103;
    USE(tmp103);
    compiler::CodeAssemblerLabel label0(&ca_);
    tmp103 = WeakToStrong_Map_0(state_, TNode<MaybeObject>{ca_.UncheckedCast<MaybeObject>(tmp101)}, &label0);
    ca_.Goto(&block19, tmp92, tmp93, tmp94, tmp95, tmp96, tmp97, tmp98, tmp99, tmp100, tmp101, ca_.UncheckedCast<MaybeObject>(tmp101), ca_.UncheckedCast<MaybeObject>(tmp101), tmp103);
    if (label0.is_used()) {
      ca_.Bind(&label0);
      ca_.Goto(&block20, tmp92, tmp93, tmp94, tmp95, tmp96, tmp97, tmp98, tmp99, tmp100, tmp101, ca_.UncheckedCast<MaybeObject>(tmp101), ca_.UncheckedCast<MaybeObject>(tmp101));
    }
  }
  ...
  if (block17.is_used()) {
    ...
    TNode<PrototypeInfo> tmp112;
    TNode<MaybeObject> tmp113;
    TNode<MaybeObject> tmp114;
    TNode<Oddball> tmp115;
    ca_.Bind(&block17, &tmp104, &tmp105, &tmp106, &tmp107, &tmp108, &tmp109, &tmp110, &tmp111, &tmp112, &tmp113, &tmp114, &tmp115);
    ca_.Goto(&block2, tmp104, tmp105, tmp106);
  }
  ...
  if (block20.is_used()) {
    ...
    TNode<PrototypeInfo> tmp124;
    TNode<MaybeObject> tmp125;
    TNode<MaybeObject> tmp126;
    TNode<MaybeObject> tmp127;
    ca_.Bind(&block20, &tmp116, &tmp117, &tmp118, &tmp119, &tmp120, &tmp121, &tmp122, &tmp123, &tmp124, &tmp125, &tmp126, &tmp127);
    ca_.Goto(&block2, tmp116, tmp117, tmp118);
  }
  ...
  if (block19.is_used()) {
    ...
    TNode<PrototypeInfo> tmp136;
    TNode<MaybeObject> tmp137;
    TNode<MaybeObject> tmp138;
    TNode<MaybeObject> tmp139;
    TNode<Map> tmp140;
    ca_.Bind(&block19, &tmp128, &tmp129, &tmp130, &tmp131, &tmp132, &tmp133, &tmp134, &tmp135, &tmp136, &tmp137, &tmp138, &tmp139, &tmp140);
    ca_.Goto(&block12, tmp128, tmp129, tmp130, tmp140, tmp132, tmp133, tmp134, tmp135);
  }
  ...
  if (block12.is_used()) {
    ...
    TNode<JSFunction> tmp148;
    ca_.Bind(&block12, &tmp141, &tmp142, &tmp143, &tmp144, &tmp145, &tmp146, &tmp147, &tmp148);
    ca_.Goto(&block3, tmp141, tmp142, tmp143, tmp144, tmp145, tmp146);
  }
  ...
  if (block3.is_used()) {
    ...
    TNode<Object> tmp154;
    ca_.Bind(&block3, &tmp149, &tmp150, &tmp151, &tmp152, &tmp153, &tmp154);
    TNode<JSObject> tmp155;
    USE(tmp155);
    tmp155 = CodeStubAssembler(state_).AllocateJSObjectFromMap(TNode<Map>{tmp152}, TNode<HeapObject>{tmp153});
  ...
  if (block2.is_used()) {
    ...
    ca_.Bind(&block2, &tmp156, &tmp157, &tmp158);
    TNode<Oddball> tmp159;
    USE(tmp159);
    tmp159 = Undefined_0(state_);
    TNode<Object> tmp160;
    tmp160 = CodeStubAssembler(state_).CallRuntime(Runtime::kObjectCreate, tmp156, tmp157, tmp159); 
    USE(tmp160);
