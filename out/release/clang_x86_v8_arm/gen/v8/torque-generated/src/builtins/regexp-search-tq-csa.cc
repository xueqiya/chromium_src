...
TNode<Object> RegExpPrototypeSearchBodyFast_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSRegExp> p_regexp, TNode<String> p_string) {
  ...
  compiler::CodeAssemblerParameterizedLabel<Context, JSRegExp, String, Smi, RegExpMatchInfo, RegExpMatchInfo, RegExpMatchInfo, IntPtrT, IntPtrT, IntPtrT, HeapObject, IntPtrT, IntPtrT, IntPtrT, IntPtrT> block14(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  compiler::CodeAssemblerParameterizedLabel<Context, JSRegExp, String, Smi, RegExpMatchInfo, RegExpMatchInfo, RegExpMatchInfo, IntPtrT, IntPtrT, IntPtrT, HeapObject, IntPtrT, IntPtrT, IntPtrT, IntPtrT> block15(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  ...
  if (block8.is_used()) {
    ...
    TNode<Smi>tmp32 = CodeStubAssembler(state_).LoadReference<Smi>(CodeStubAssembler::Reference{tmp28, tmp31});
    TNode<IntPtrT> tmp33;
    USE(tmp33);
    tmp33 = Convert_intptr_Smi_0(state_, TNode<Smi>{tmp32});
    TNode<IntPtrT> tmp34;
    USE(tmp34);
    tmp34 = Convert_intptr_constexpr_int31_0(state_, RegExpMatchInfo::kFirstCaptureIndex);
    ...
    ca_.Branch(tmp37, &block14, &block15, tmp21, tmp22, tmp23, tmp24, tmp28, tmp28, tmp28, tmp29, tmp33, tmp34, tmp28, tmp29, tmp33, tmp34, tmp34);
  }
  ...
  if (block14.is_used()) {
    ...
    TNode<IntPtrT> tmp51;
    TNode<IntPtrT> tmp52;
    ca_.Bind(&block14, &tmp38, &tmp39, &tmp40, &tmp41, &tmp42, &tmp43, &tmp44, &tmp45, &tmp46, &tmp47, &tmp48, &tmp49, &tmp50, &tmp51, &tmp52);
    TNode<IntPtrT> tmp53;
    USE(tmp53);
    ...
    TNode<IntPtrT> tmp57;
    USE(tmp57);
    std::tie(tmp56, tmp57) = UnsafeNewReference_Object_0(state_, TNode<HeapObject>{tmp48}, TNode<IntPtrT>{tmp55}).Flatten();
    TNode<Object>tmp58 = CodeStubAssembler(state_).LoadReference<Object>(CodeStubAssembler::Reference{tmp56, tmp57});
    TNode<Smi> tmp59;
    USE(tmp59);
    tmp59 = UnsafeCast_Smi_0(state_, TNode<Context>{tmp38}, TNode<Object>{tmp58});
    ca_.Goto(&block1, tmp38, tmp39, tmp40, tmp59);
  }
  ...
  if (block15.is_used()) {
    ...
    TNode<IntPtrT> tmp73;
    TNode<IntPtrT> tmp74;
    ca_.Bind(&block15, &tmp60, &tmp61, &tmp62, &tmp63, &tmp64, &tmp65, &tmp66, &tmp67, &tmp68, &tmp69, &tmp70, &tmp71, &tmp72, &tmp73, &tmp74);
    CodeStubAssembler(state_).Unreachable();
  }

  if (block1.is_used()) {
    TNode<Context> tmp75;
    TNode<JSRegExp> tmp76;
    TNode<String> tmp77;
    TNode<Object> tmp78;
    ca_.Bind(&block1, &tmp75, &tmp76, &tmp77, &tmp78);
    ca_.Goto(&block17, tmp75, tmp76, tmp77, tmp78);
  }

    TNode<Context> tmp79;
    TNode<JSRegExp> tmp80;
    TNode<String> tmp81;
    TNode<Object> tmp82;
    ca_.Bind(&block17, &tmp79, &tmp80, &tmp81, &tmp82);
