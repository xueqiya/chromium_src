...
TF_BUILTIN(StringToList, CodeStubAssembler) {
  ...
  if (block0.is_used()) {
    ...
    TNode<Map> tmp4;
    USE(tmp4);
    tmp4 = CodeStubAssembler(state_).LoadJSArrayElementsMap(ElementsKind::PACKED_ELEMENTS, TNode<NativeContext>{tmp3});
    TNode<Smi> tmp5;
    USE(tmp5);
    tmp5 = CodeStubAssembler(state_).SmiTag(TNode<IntPtrT>{tmp2});
    TNode<JSArray> tmp6;
    USE(tmp6);
    tmp6 = CodeStubAssembler(state_).AllocateJSArray(ElementsKind::PACKED_ELEMENTS, TNode<Map>{tmp4}, TNode<IntPtrT>{tmp2}, TNode<Smi>{tmp5}, CodeStubAssembler::AllocationFlag::kAllowLargeObjectAllocation);
    TNode<IntPtrT> tmp7;
    USE(tmp7);
  ...
  if (block2.is_used()) {
    ...
    CodeStubAssembler(state_).StoreReference<Number>(CodeStubAssembler::Reference{tmp42, tmp46}, tmp44);
  ...
  if (block7.is_used()) {
    ...
    CodeStubAssembler(state_).FailAssert("Torque assert 'arrayLength >= 0' failed", "src/builtins/builtins-string.tq", 55);
  }
  ...
  if (block11.is_used()) {
    ...
    CodeStubAssembler(state_).FailAssert("Torque assert 'SmiTag(stringLength) >= arrayLength' failed", "src/builtins/builtins-string.tq", 56);
  }
...
void GenerateStringAt_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<Object> p_receiver, TNode<Object> p_position, const char* p_methodName, compiler::CodeAssemblerLabel* label_IfInBounds, compiler::TypedCodeAssemblerVariable<String>* label_IfInBounds_parameter_0, compiler::TypedCodeAssemblerVariable<UintPtrT>* label_IfInBounds_parameter_1, compiler::TypedCodeAssemblerVariable<UintPtrT>* label_IfInBounds_parameter_2, compiler::CodeAssemblerLabel* label_IfOutOfBounds) {
  ...
  if (block12.is_used()) {
    ...
    CodeStubAssembler(state_).FailAssert("Torque assert 'IsNumberNormalized(indexHeapNumber)' failed", "src/builtins/builtins-string.tq", 88);
  }
...
USE(parameter3);
  ...
  if (block11.is_used()) {
    ...
    CodeStubAssembler(state_).StoreReference<Object>(CodeStubAssembler::Reference{tmp113, tmp114}, tmp109);
