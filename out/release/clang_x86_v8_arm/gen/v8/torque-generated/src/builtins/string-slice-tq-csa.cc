...
USE(parameter1);
  ...
  if (block0.is_used()) {
    ...
    TNode<String> tmp5;
    USE(tmp5);
    tmp5 = FromConstexpr_String_constexpr_string_0(state_, "String.prototype.slice");
    TNode<String> tmp6;
    USE(tmp6);
  ...
  if (block7.is_used()) {
    ...
    TNode<BoolT> tmp75;
    USE(tmp75);
    tmp75 = CodeStubAssembler(state_).UintPtrLessThanOrEqual(TNode<UintPtrT>{tmp74}, TNode<UintPtrT>{tmp72});
  ...
  if (block10.is_used()) {
    ...
    ca_.Bind(&block10, &tmp88, &tmp89, &tmp90, &tmp91, &tmp92, &tmp93, &tmp94, &tmp95, &tmp96, &tmp97, &tmp98);
    TNode<String> tmp99;
    USE(tmp99);
    tmp99 = StringBuiltinsAssembler(state_).SubString(TNode<String>{tmp93}, TNode<UintPtrT>{tmp96}, TNode<UintPtrT>{tmp98});
