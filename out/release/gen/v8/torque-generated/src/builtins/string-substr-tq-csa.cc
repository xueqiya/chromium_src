...
USE(parameter1);
  ...
  compiler::CodeAssemblerParameterizedLabel<RawPtrT, RawPtrT, IntPtrT, NativeContext, Object, String, UintPtrT, Object, UintPtrT, Object, UintPtrT> block8(&ca_, compiler::CodeAssemblerLabel::kDeferred);
  ...
  compiler::CodeAssemblerParameterizedLabel<RawPtrT, RawPtrT, IntPtrT, NativeContext, Object, String, UintPtrT, Object, UintPtrT, Object, UintPtrT, UintPtrT> block11(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  compiler::CodeAssemblerParameterizedLabel<RawPtrT, RawPtrT, IntPtrT, NativeContext, Object, String, UintPtrT, Object, UintPtrT, Object, UintPtrT, UintPtrT> block13(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  compiler::CodeAssemblerParameterizedLabel<RawPtrT, RawPtrT, IntPtrT, NativeContext, Object, String, UintPtrT, Object, UintPtrT, Object, UintPtrT, UintPtrT> block14(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  ...
  if (block0.is_used()) {
    ...
    TNode<String> tmp5;
    USE(tmp5);
    tmp5 = FromConstexpr_String_constexpr_string_0(state_, "String.prototype.substr");
    TNode<String> tmp6;
    USE(tmp6);
  ...
  if (block3.is_used()) {
    ...
    USE(tmp41);
    tmp41 = CodeStubAssembler(state_).UintPtrSub(TNode<UintPtrT>{tmp36}, TNode<UintPtrT>{tmp38});
    TNode<Oddball> tmp42;
    USE(tmp42);
    tmp42 = Undefined_0(state_);
    TNode<BoolT> tmp43;
    USE(tmp43);
    tmp43 = CodeStubAssembler(state_).TaggedNotEqual(TNode<Object>{tmp40}, TNode<HeapObject>{tmp42});
    ca_.Branch(tmp43, &block9, &block10, tmp30, tmp31, tmp32, tmp33, tmp34, tmp35, tmp36, tmp37, tmp38, tmp40, tmp41);
  }
  ...
  if (block8.is_used()) {
    ...
    ca_.Bind(&block8, &tmp44, &tmp45, &tmp46, &tmp47, &tmp48, &tmp49, &tmp50, &tmp51, &tmp52, &tmp53, &tmp54);
    CodeStubAssembler(state_).FailAssert("Torque assert 'lengthLimit <= size' failed", "src/builtins/string-substr.tq", 31);
  }
  ...
  if (block7.is_used()) {
    ...
    ca_.Bind(&block7, &tmp55, &tmp56, &tmp57, &tmp58, &tmp59, &tmp60, &tmp61, &tmp62, &tmp63, &tmp64, &tmp65);
  }
  ...
  if (block9.is_used()) {
    ...
    ca_.Bind(&block9, &tmp66, &tmp67, &tmp68, &tmp69, &tmp70, &tmp71, &tmp72, &tmp73, &tmp74, &tmp75, &tmp76);
    TNode<UintPtrT> tmp77;
    USE(tmp77);
    tmp77 = ClampToIndexRange_0(state_, TNode<Context>{tmp69}, TNode<Object>{tmp75}, TNode<UintPtrT>{tmp76});
    ca_.Goto(&block11, tmp66, tmp67, tmp68, tmp69, tmp70, tmp71, tmp72, tmp73, tmp74, tmp75, tmp76, tmp77);
  }
  ...
  if (block10.is_used()) {
    ...
    ca_.Bind(&block10, &tmp78, &tmp79, &tmp80, &tmp81, &tmp82, &tmp83, &tmp84, &tmp85, &tmp86, &tmp87, &tmp88);
    ca_.Goto(&block11, tmp78, tmp79, tmp80, tmp81, tmp82, tmp83, tmp84, tmp85, tmp86, tmp87, tmp88, tmp88);
  }
  ...
  if (block11.is_used()) {
    ...
    ca_.Bind(&block11, &tmp89, &tmp90, &tmp91, &tmp92, &tmp93, &tmp94, &tmp95, &tmp96, &tmp97, &tmp98, &tmp99, &tmp100);
    TNode<UintPtrT> tmp101;
    USE(tmp101);
    tmp101 = FromConstexpr_uintptr_constexpr_int31_0(state_, 0);
    TNode<BoolT> tmp102;
    USE(tmp102);
    tmp102 = CodeStubAssembler(state_).WordEqual(TNode<UintPtrT>{tmp100}, TNode<UintPtrT>{tmp101});
    ca_.Branch(tmp102, &block13, &block14, tmp89, tmp90, tmp91, tmp92, tmp93, tmp94, tmp95, tmp96, tmp97, tmp98, tmp99, tmp100);
  }
  ...
  if (block13.is_used()) {
    ...
    ca_.Bind(&block13, &tmp103, &tmp104, &tmp105, &tmp106, &tmp107, &tmp108, &tmp109, &tmp110, &tmp111, &tmp112, &tmp113, &tmp114);
    TNode<String> tmp115;
    USE(tmp115);
    tmp115 = CodeStubAssembler(state_).EmptyStringConstant();
  ...
  if (block14.is_used()) {
    ...
    ca_.Bind(&block14, &tmp116, &tmp117, &tmp118, &tmp119, &tmp120, &tmp121, &tmp122, &tmp123, &tmp124, &tmp125, &tmp126, &tmp127);
    TNode<UintPtrT> tmp128;
    USE(tmp128);
    tmp128 = CodeStubAssembler(state_).UintPtrAdd(TNode<UintPtrT>{tmp124}, TNode<UintPtrT>{tmp127});
    TNode<String> tmp129;
    USE(tmp129);
    tmp129 = StringBuiltinsAssembler(state_).SubString(TNode<String>{tmp121}, TNode<UintPtrT>{tmp124}, TNode<UintPtrT>{tmp128});
    arguments.PopAndReturn(tmp129);
  }
}
