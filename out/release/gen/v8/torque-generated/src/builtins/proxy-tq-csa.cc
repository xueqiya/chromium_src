...
TNode<BoolT> IsRevokedProxy_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSReceiver> p_o) {
  ...
  if (block4.is_used()) {
    ...
    TNode<HeapObject>tmp12 = CodeStubAssembler(state_).LoadReference<HeapObject>(CodeStubAssembler::Reference{tmp10, tmp11});
    TNode<JSReceiver> tmp13;
    USE(tmp13);
    compiler::CodeAssemblerLabel label0(&ca_);
    tmp13 = Cast_JSReceiver_0(state_, TNode<HeapObject>{tmp12}, &label0);
    ca_.Goto(&block8, tmp7, tmp8, tmp10, tmp12, tmp13);
    if (label0.is_used()) {
      ca_.Bind(&label0);
      ca_.Goto(&block9, tmp7, tmp8, tmp10, tmp12);
    }
  }
