...
TF_BUILTIN(ProxyDeleteProperty, CodeStubAssembler) {
  ...
  compiler::CodeAssemblerParameterizedLabel<Context, JSProxy, Name, Smi, JSReceiver, JSReceiver, JSReceiver, Object, Smi> block32(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  compiler::CodeAssemblerParameterizedLabel<Context, JSProxy, Name, Smi, JSReceiver, JSReceiver, JSReceiver, Object, Smi> block33(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  ...
  if (block28.is_used()) {
    ...
    TNode<Object> tmp92;
    USE(tmp92);
    tmp92 = CodeStubAssembler(state_).Call(TNode<Context>{tmp83}, TNode<Object>{tmp91}, TNode<Object>{tmp87}, TNode<Object>{tmp88}, TNode<Object>{tmp85});
    TNode<BoolT> tmp93;
    USE(tmp93);
  ...
  if (block30.is_used()) {
    ...
    TNode<Smi> tmp103;
    USE(tmp103);
    tmp103 = FromConstexpr_LanguageModeSmi_constexpr_LanguageMode_0(state_, LanguageMode::kStrict);
    TNode<BoolT> tmp104;
    USE(tmp104);
    tmp104 = CodeStubAssembler(state_).SmiEqual(TNode<Smi>{tmp98}, TNode<Smi>{tmp103});
    ca_.Branch(tmp104, &block32, &block33, tmp95, tmp96, tmp97, tmp98, tmp99, tmp100, tmp101, tmp102, tmp103);
  }
  ...
  if (block32.is_used()) {
    ...
    TNode<Object> tmp112;
    TNode<Smi> tmp113;
    ca_.Bind(&block32, &tmp105, &tmp106, &tmp107, &tmp108, &tmp109, &tmp110, &tmp111, &tmp112, &tmp113);
    TNode<Object> tmp114;
    USE(tmp114);
    tmp114 = FromConstexpr_Object_constexpr_string_0(state_, "deleteProperty");
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{tmp105}, MessageTemplate::kProxyTrapReturnedFalsishFor, TNode<Object>{tmp114}, TNode<Object>{tmp107});
  }
  ...
  if (block33.is_used()) {
    ...
    TNode<Object> tmp122;
    TNode<Smi> tmp123;
    ca_.Bind(&block33, &tmp115, &tmp116, &tmp117, &tmp118, &tmp119, &tmp120, &tmp121, &tmp122, &tmp123);
    TNode<Oddball> tmp124;
    USE(tmp124);
    tmp124 = False_0(state_);
  ...
  if (block31.is_used()) {
    ...
    TNode<Object> tmp132;
    ca_.Bind(&block31, &tmp125, &tmp126, &tmp127, &tmp128, &tmp129, &tmp130, &tmp131, &tmp132);
    ProxiesCodeStubAssembler(state_).CheckDeleteTrapResult(TNode<Context>{tmp125}, TNode<JSReceiver>{tmp130}, TNode<JSProxy>{tmp126}, TNode<Name>{tmp127});
