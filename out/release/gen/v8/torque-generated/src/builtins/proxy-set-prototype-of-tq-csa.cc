...
TF_BUILTIN(ProxySetPrototypeOf, CodeStubAssembler) {
  ...
  if (block23.is_used()) {
    ...
    TNode<Object> tmp94;
    USE(tmp94);
    tmp94 = CodeStubAssembler(state_).Call(TNode<Context>{tmp85}, TNode<Object>{tmp93}, TNode<Object>{tmp89}, TNode<Object>{tmp90}, TNode<Object>{tmp87});
    TNode<BoolT> tmp95;
    USE(tmp95);
  ...
  if (block32.is_used()) {
    ...
    CodeStubAssembler(state_).FailAssert("Torque assert 'extensibleTarget == True || extensibleTarget == False' failed", "src/builtins/proxy-set-prototype-of.tq", 51);
  }
