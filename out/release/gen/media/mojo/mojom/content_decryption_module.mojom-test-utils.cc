...
void ContentDecryptionModuleAsyncWaiter::Initialize(
    const std::string& key_system, const ::url::Origin& security_origin, const ::media::CdmConfig& cdm_config, CdmPromiseResultPtr* out_result, int32_t* out_cdm_id, mojo::PendingRemote<::media::mojom::Decryptor>* out_decryptor) {
  base::RunLoop loop;
...
,
             ...
             int32_t cdm_id,
             mojo::PendingRemote<::media::mojom::Decryptor> decryptor) {*out_result = std::move(result);*out_cdm_id = std::move(cdm_id);*out_decryptor = std::move(decryptor);
            loop->Quit();
          },
          &loop,
          out_result,
...
void CdmFactoryInterceptorForTesting::CreateCdm(const std::string& key_system, mojo::PendingReceiver<ContentDecryptionModule> cdm) {
