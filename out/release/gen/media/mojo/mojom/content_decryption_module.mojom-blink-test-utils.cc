...
void ContentDecryptionModuleAsyncWaiter::Initialize(
    const WTF::String& key_system, const ::scoped_refptr<const ::blink::SecurityOrigin>& security_origin, CdmConfigPtr cdm_config, CdmPromiseResultPtr* out_result, int32_t* out_cdm_id, mojo::PendingRemote<::media::mojom::blink::Decryptor>* out_decryptor) {
  base::RunLoop loop;
...
,
             int32_t* out_cdm_id
,
             mojo::PendingRemote<::media::mojom::blink::Decryptor>* out_decryptor
,
             CdmPromiseResultPtr result,
             int32_t cdm_id,
             mojo::PendingRemote<::media::mojom::blink::Decryptor> decryptor) {*out_result = std::move(result);*out_cdm_id = std::move(cdm_id);*out_decryptor = std::move(decryptor);
            loop->Quit();
          },
          &loop,
          out_result,
...
void CdmFactoryInterceptorForTesting::CreateCdm(const WTF::String& key_system, mojo::PendingReceiver<ContentDecryptionModule> cdm) {
