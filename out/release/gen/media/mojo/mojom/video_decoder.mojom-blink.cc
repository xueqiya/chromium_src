...
void VideoDecoderProxy::Construct(
    mojo::PendingAssociatedRemote<VideoDecoderClient> in_client, mojo::PendingAssociatedRemote<::media::mojom::blink::MediaLog> in_media_log, mojo::PendingReceiver<VideoFrameHandleReleaser> in_video_frame_handle_releaser, mojo::ScopedDataPipeConsumerHandle in_decoder_buffer_pipe, CommandBufferIdPtr in_command_buffer_id, VideoDecoderImplementation in_implementation, const ::gfx::ColorSpace& in_target_color_space) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT1("mojom", "(Impl)media::mojom::VideoDecoder::GetSupportedConfigsCallback",
               "message", message.name());
#endif
...
bool VideoDecoderStubDispatch::Accept(
  ...
  switch (message->header()->name) {
    ...
    case internal::kVideoDecoder_Construct_Name: {
      ...
      mojo::PendingAssociatedRemote<::media::mojom::blink::MediaLog> p_media_log{};
      ...
      if (success && !input_data_view.ReadCommandBufferId(&p_command_buffer_id))
        success = false;
      if (success && !input_data_view.ReadImplementation(&p_implementation))
        success = false;
...
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT0("mojom", "media::mojom::VideoDecoderClient::OnWaiting");
#endif
  const bool kExpectsResponse = false;
  const bool kIsSync = false;
