...
class BLINK_PLATFORM_EXPORT KeySystemCapability {
 public:
  ...
  KeySystemCapability(
      ...
      WTF::Vector<::media::mojom::blink::EncryptionScheme> hw_secure_encryption_schemes,
...
bool operator<(const T& lhs, const T& rhs) {
  ...
  if (rhs.video_codecs < lhs.video_codecs)
    return false;
  if (lhs.supports_vp9_profile2 < rhs.supports_vp9_profile2)
    return true;
  if (rhs.supports_vp9_profile2 < lhs.supports_vp9_profile2)
    return false;
  if (lhs.encryption_schemes < rhs.encryption_schemes)
    return true;
  if (rhs.encryption_schemes < lhs.encryption_schemes)
    return false;
  if (lhs.hw_secure_video_codecs < rhs.hw_secure_video_codecs)
    return true;
  if (rhs.hw_secure_video_codecs < lhs.hw_secure_video_codecs)
    return false;
  if (lhs.hw_secure_encryption_schemes < rhs.hw_secure_encryption_schemes)
    return true;
  if (rhs.hw_secure_encryption_schemes < lhs.hw_secure_encryption_schemes)
    return false;
