...
void VideoCaptureHostProxy::ReleaseBuffer(
    const ::base::UnguessableToken& in_device_id, int32_t in_buffer_id, double in_consumer_resource_utilization) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT0("mojom", "media::mojom::VideoCaptureHost::OnFrameDropped");
#endif
  const bool kExpectsResponse = false;
  const bool kIsSync = false;
...
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT0("mojom", "media::mojom::VideoCaptureHost::OnLog");
#endif
  const bool kExpectsResponse = false;
  const bool kIsSync = false;
