...
class  ScrollOffset {
 public:
  template <typename T>
  using EnableIfSame = std::enable_if_t<std::is_same<ScrollOffset, T>::value>;
  ...
  // bindings generator does not need to know whether Equals() or == operator
  // are available for members.
  template <typename T, ScrollOffset::EnableIfSame<T>* = nullptr>
  bool Equals(const T& other) const;
  size_t Hash(size_t seed) const;
...
class  Quaternion {
 public:
  ...
  static WTF::Vector<uint8_t> Serialize(UserType* input) {
    return mojo::internal::SerializeImpl<
        Quaternion::DataView, WTF::Vector<uint8_t>>(input);
  }
...
template <typename T, ScrollOffset::EnableIfSame<T>*>
