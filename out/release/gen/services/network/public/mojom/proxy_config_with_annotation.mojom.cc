...
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT1("mojom", "(Impl)network::mojom::ProxyConfigClient::FlushProxyConfigCallback",
               "message", message.name());
#endif
...
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT0("mojom", "network::mojom::ProxyErrorClient::OnPACScriptError");
#endif
  const bool kExpectsResponse = false;
  const bool kIsSync = false;
...
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT0("mojom", "network::mojom::ProxyErrorClient::OnRequestMaybeFailedDueToProxySettings");
#endif
  const bool kExpectsResponse = false;
  const bool kIsSync = false;
