...
// static
bool WebSocketClient_AddSendFlowControlQuota_Params_Data::Validate(
    const void* data,
    mojo::internal::ValidationContext* validation_context) {
  if (!data)
    return true;
  ...
  // NOTE: The memory backing |object| may be smaller than |sizeof(*object)| if
  // the message comes from an older version.
  const WebSocketClient_AddSendFlowControlQuota_Params_Data* object = static_cast<const WebSocketClient_AddSendFlowControlQuota_Params_Data*>(data);
...
WebSocketClient_AddSendFlowControlQuota_Params_Data::WebSocketClient_AddSendFlowControlQuota_Params_Data()
    : header_({sizeof(*this), 0}) {}
