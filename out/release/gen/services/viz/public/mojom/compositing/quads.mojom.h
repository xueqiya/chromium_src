...
class  RenderPassQuadState {
 public:
  template <typename T>
  using EnableIfSame = std::enable_if_t<std::is_same<RenderPassQuadState, T>::value>;
  ...
  // bindings generator does not need to know whether Equals() or == operator
  // are available for members.
  template <typename T, RenderPassQuadState::EnableIfSame<T>* = nullptr>
  bool Equals(const T& other) const;
...
template <typename T, RenderPassQuadState::EnableIfSame<T>*>
