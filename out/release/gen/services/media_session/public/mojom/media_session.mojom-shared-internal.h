 ...
 public:
  class BufferWriter {
   public:
    ...
    void Allocate(mojo::internal::Buffer* serialization_buffer) {
      serialization_buffer_ = serialization_buffer;
      index_ = serialization_buffer_->Allocate(sizeof(MediaPosition_Data));
      new (data()) MediaPosition_Data();
    }

    bool is_null() const { return !serialization_buffer_; }
    MediaPosition_Data* data() {
      DCHECK(!is_null());
      return serialization_buffer_->Get<MediaPosition_Data>(index_);
    }
    MediaPosition_Data* operator->() { return data(); }
...
static_assert(sizeof(MediaPosition_Data) == 40,
...
struct MediaPosition_UnserializedMessageContext
 ...
 private:
  ...
  void Serialize(mojo::internal::SerializationContext* context,
                 mojo::internal::Buffer* buffer) override {
    MediaPosition_Data::BufferWriter writer;
    mojo::internal::Serialize<DataView>(user_data_, buffer, &writer, context);
  }
