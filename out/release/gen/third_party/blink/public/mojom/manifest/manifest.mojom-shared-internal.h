 ...
 public:
  class BufferWriter {
   public:
    ...
    void Allocate(mojo::internal::Buffer* serialization_buffer) {
      serialization_buffer_ = serialization_buffer;
      index_ = serialization_buffer_->Allocate(sizeof(ManifestShortcutItem_Data));
      new (data()) ManifestShortcutItem_Data();
    }

    bool is_null() const { return !serialization_buffer_; }
    ManifestShortcutItem_Data* data() {
      DCHECK(!is_null());
      return serialization_buffer_->Get<ManifestShortcutItem_Data>(index_);
    }
    ManifestShortcutItem_Data* operator->() { return data(); }
...
struct ManifestShortcutItem_UnserializedMessageContext
 ...
 private:
  ...
  void Serialize(mojo::internal::SerializationContext* context,
                 mojo::internal::Buffer* buffer) override {
    ManifestShortcutItem_Data::BufferWriter writer;
    mojo::internal::Serialize<DataView>(user_data_, buffer, &writer, context);
  }
...
static_assert(sizeof(ManifestFileHandler_Data) == 32,
