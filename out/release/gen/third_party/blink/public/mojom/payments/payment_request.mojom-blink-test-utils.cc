// third_party/blink/public/mojom/payments/payment_request.mojom-blink-test-utils.cc is auto generated by mojom_bindings_generator.py, do not edit
...
void PaymentRequestInterceptorForTesting::Init(mojo::PendingRemote<PaymentRequestClient> client, WTF::Vector<PaymentMethodDataPtr> method_data, PaymentDetailsPtr details, PaymentOptionsPtr options, bool google_pay_bridge_eligible) {
  GetForwardingInterface()->Init(std::move(client), std::move(method_data), std::move(details), std::move(options), std::move(google_pay_bridge_eligible));
}
