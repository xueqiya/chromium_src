 ...
 public:
  class BufferWriter {
   public:
    ...
    void Allocate(mojo::internal::Buffer* serialization_buffer) {
      serialization_buffer_ = serialization_buffer;
      index_ = serialization_buffer_->Allocate(sizeof(Portal_Navigate_ResponseParams_Data));
      new (data()) Portal_Navigate_ResponseParams_Data();
    }

    bool is_null() const { return !serialization_buffer_; }
    Portal_Navigate_ResponseParams_Data* data() {
      DCHECK(!is_null());
      return serialization_buffer_->Get<Portal_Navigate_ResponseParams_Data>(index_);
    }
    Portal_Navigate_ResponseParams_Data* operator->() { return data(); }
...
class Portal_Activate_ResponseParamsDataView {
 public:
  ...
  PortalActivateResult result() const {
    return static_cast<PortalActivateResult>(data_->result);
  }
 private:
