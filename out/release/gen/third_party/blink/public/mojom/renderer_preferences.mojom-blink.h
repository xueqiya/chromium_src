...
bool operator<(const T& lhs, const T& rhs) {
  ...
  if (rhs.accept_languages < lhs.accept_languages)
    return false;
  if (lhs.disable_client_blocked_error_page < rhs.disable_client_blocked_error_page)
    return true;
  if (rhs.disable_client_blocked_error_page < lhs.disable_client_blocked_error_page)
    return false;
