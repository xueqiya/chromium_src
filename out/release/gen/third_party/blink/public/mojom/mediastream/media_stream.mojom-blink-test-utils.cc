...
,
             ...
             const WTF::Vector<::blink::MediaStreamDevice>& video_devices) {*out_result = std::move(result);*out_label = std::move(label);*out_audio_devices = std::move(audio_devices);*out_video_devices = std::move(video_devices);
            loop->Quit();
          },
          &loop,
          out_result,
