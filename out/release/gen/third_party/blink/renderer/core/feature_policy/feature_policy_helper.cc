...
const char kPublicKeyCredentialsPolicyName[] = "publickey-credentials";
...
const FeatureNameMap& GetDefaultFeatureNameMap() {
  ...
  if (default_feature_name_map.IsEmpty()) {
    ...
    if (RuntimeEnabledFeatures::WebAuthenticationFeaturePolicyEnabled()) {
      default_feature_name_map.Set(kPublicKeyCredentialsPolicyName,
...
const FeatureSet& GetAvailableDocumentPolicyFeatures() {
  ...
  if (features.IsEmpty()) {
    features.insert(mojom::FeaturePolicyFeature::kUnoptimizedLosslessImages);
    if (RuntimeEnabledFeatures::ExperimentalProductivityFeaturesEnabled()) {
      features.insert(mojom::FeaturePolicyFeature::kFontDisplay);
    }
  }
  return features;
}
...
bool DisabledByOriginTrial(const String& feature_name,
  ...
  if (feature_name == kUnsizedMediaPolicyName) {
    return !RuntimeEnabledFeatures::UnsizedMediaPolicyEnabled(feature_context);
  }
  if (feature_name == kWakeLockPolicyName) {
