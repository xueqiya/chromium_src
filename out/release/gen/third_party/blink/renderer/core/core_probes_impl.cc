...
void CoreProbeSink::AddInspectorMediaAgent(InspectorMediaAgent* agent) {
  ...
  inspector_media_agents_.insert(agent);
...
void CoreProbeSink::RemoveInspectorMediaAgent(InspectorMediaAgent* agent) {
  ...
  inspector_media_agents_.erase(agent);
...
void DidFinishLoadingImpl(CoreProbeSink* param_core_probe_sink, uint64_t identifier, DocumentLoader* param_document_loader, base::TimeTicks finish_time, int64_t encoded_data_length, int64_t decoded_body_length, bool should_report_corb_blocking) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_core_probe_sink);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorTraceEventss()) {
...
void FileChooserOpenedImpl(LocalFrame* frame, HTMLInputElement* element, bool* intercepted) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(frame);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorPageAgents()) {
    for (InspectorPageAgent* agent : probe_sink->InspectorPageAgents())
      agent->FileChooserOpened(frame, element, intercepted);
  }
}

void PlayerEventsAddedImpl(LocalFrame* frame, String player_id, const Vector<InspectorPlayerEvent>& events) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(frame);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorMediaAgents()) {
    for (InspectorMediaAgent* agent : probe_sink->InspectorMediaAgents())
      agent->PlayerEventsAdded(player_id, events);
  }
}

void PlayerPropertiesChangedImpl(LocalFrame* frame, String player_id, const Vector<InspectorPlayerProperty>& properties) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(frame);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorMediaAgents()) {
...
void PlayersCreatedImpl(LocalFrame* frame, const Vector<WebString>& players) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(frame);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorMediaAgents()) {
    for (InspectorMediaAgent* agent : probe_sink->InspectorMediaAgents())
      agent->PlayersCreated(players);
  }
}
