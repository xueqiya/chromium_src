...
class CORE_EXPORT WindowPostMessageOptions : public PostMessageOptions {
 public:
  ...
  bool hasAllow() const { return !allow_.IsNull(); }
  const String& allow() const {
    return allow_;
  }
  inline void setAllow(const String&);
...
void WindowPostMessageOptions::setAllow(const String& value) {
  allow_ = value;
}
