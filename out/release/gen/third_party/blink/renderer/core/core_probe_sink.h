...
class CORE_EXPORT CoreProbeSink final : public GarbageCollected<CoreProbeSink> {
 ...
 public:
  enum AgentType : unsigned {
    ...
    kInspectorTraceEvents = 1u << 11,
    kInspectorPerformanceAgent = 1u << 12,
    kInspectorAnimationAgent = 1u << 13,
    kInspectorPageAgent = 1u << 14,
    kInspectorDOMSnapshotAgent = 1u << 15,
    kInspectorLayerTreeAgent = 1u << 16,
  };
  ...
  const HeapListHashSet<Member<InspectorMediaAgent>>& InspectorMediaAgents() const { return inspector_media_agents_; }
 ...
 private:
  ...
  HeapListHashSet<Member<InspectorMediaAgent>> inspector_media_agents_;
