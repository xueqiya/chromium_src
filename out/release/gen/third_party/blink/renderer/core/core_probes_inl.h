...
inline void DidFinishLoading(CoreProbeSink* param_core_probe_sink, uint64_t identifier, DocumentLoader* param_document_loader, base::TimeTicks finish_time, int64_t encoded_data_length, int64_t decoded_body_length, bool should_report_corb_blocking) {
  if (!CoreProbeSink::HasAgentsGlobal(CoreProbeSink::kInspectorTraceEvents | CoreProbeSink::kInspectorNetworkAgent))
    return;
...
CORE_EXPORT void FileChooserOpenedImpl(LocalFrame*, HTMLInputElement*, bool*);
inline void FileChooserOpened(LocalFrame* frame, HTMLInputElement* element, bool* intercepted) {
  if (!CoreProbeSink::HasAgentsGlobal(CoreProbeSink::kInspectorPageAgent))
    return;

  FileChooserOpenedImpl(frame, element, intercepted);
}

CORE_EXPORT void PlayerEventsAddedImpl(LocalFrame*, String, const Vector<InspectorPlayerEvent>&);
inline void PlayerEventsAdded(LocalFrame* frame, String player_id, const Vector<InspectorPlayerEvent>& events) {
  if (!CoreProbeSink::HasAgentsGlobal(CoreProbeSink::kInspectorMediaAgent))
    return;

  PlayerEventsAddedImpl(frame, player_id, events);
}

CORE_EXPORT void PlayerPropertiesChangedImpl(LocalFrame*, String, const Vector<InspectorPlayerProperty>&);
inline void PlayerPropertiesChanged(LocalFrame* frame, String player_id, const Vector<InspectorPlayerProperty>& properties) {
  if (!CoreProbeSink::HasAgentsGlobal(CoreProbeSink::kInspectorMediaAgent))
    return;

  PlayerPropertiesChangedImpl(frame, player_id, properties);
}

CORE_EXPORT void PlayersCreatedImpl(LocalFrame*, const Vector<WebString>&);
inline void PlayersCreated(LocalFrame* frame, const Vector<WebString>& players) {
  if (!CoreProbeSink::HasAgentsGlobal(CoreProbeSink::kInspectorMediaAgent))
    return;

  PlayersCreatedImpl(frame, players);
}
