...
static void AutocompleteAttributeGetter(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (cpp_value.IsEmpty()) {
    cpp_value = keywords::kOn;
  } else if (atomic_cpp_value == keywords::kOn) {
    cpp_value = keywords::kOn;
  } else if (atomic_cpp_value == keywords::kOff) {
    cpp_value = keywords::kOff;
  } else {
    cpp_value = keywords::kOn;
  }
...
static void RequestSubmitMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ExceptionState exception_state(info.GetIsolate(), ExceptionState::kExecutionContext, "HTMLFormElement", "requestSubmit");
  ...
  if (UNLIKELY(num_args_passed <= 0)) {
    impl->requestSubmit(exception_state);
    if (exception_state.HadException()) {
      return;
    }
    return;
  }
  submitter = V8HTMLElement::ToImplWithTypeCheck(info.GetIsolate(), info[0]);
  if (!submitter) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(0, "HTMLElement"));
    return;
  }

  impl->requestSubmit(submitter, exception_state);
  if (exception_state.HadException()) {
    return;
  }
}
...
void V8HTMLFormElement::AcceptCharsetAttributeSetterCallback(
  ...
  V8SetReflectedDOMStringAttribute(info, html_names::kAcceptCharsetAttr);
}
...
void V8HTMLFormElement::NoValidateAttributeSetterCallback(
  ...
  V8SetReflectedBooleanAttribute(info, "HTMLFormElement", "noValidate", html_names::kNovalidateAttr);
}
...
void V8HTMLFormElement::RequestSubmitMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  html_form_element_v8_internal::RequestSubmitMethod(info);
}
...
static constexpr V8DOMConfiguration::MethodConfiguration kV8HTMLFormElementMethods[] = {
    ...
    {"requestSubmit", V8HTMLFormElement::RequestSubmitMethodCallback, 0, v8::None, V8DOMConfiguration::kOnPrototype, V8DOMConfiguration::kCheckHolder, V8DOMConfiguration::kDoNotCheckAccess, V8DOMConfiguration::kHasSideEffect, V8DOMConfiguration::kAllWorlds},
