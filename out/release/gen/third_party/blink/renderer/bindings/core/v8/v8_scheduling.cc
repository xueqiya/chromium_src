...
static void IsFramePendingMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  Scheduling* impl = V8Scheduling::ToImpl(info.Holder());

  V8SetReturnValueBool(info, impl->isFramePending());
}
...
void V8Scheduling::IsFramePendingMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  BLINK_BINDINGS_TRACE_EVENT("Scheduling.isFramePending");
  RUNTIME_CALL_TIMER_SCOPE_DISABLED_BY_DEFAULT(info.GetIsolate(), "Blink_Scheduling_isFramePending");

  scheduling_v8_internal::IsFramePendingMethod(info);
}
...
void V8Scheduling::InstallExperimentalIsInputPending(
  ...
  kIsFramePendingConfigurations[] = {
      {"isFramePending", V8Scheduling::IsFramePendingMethodCallback, 0, v8::None, V8DOMConfiguration::kOnPrototype, V8DOMConfiguration::kCheckHolder, V8DOMConfiguration::kDoNotCheckAccess, V8DOMConfiguration::kHasSideEffect, V8DOMConfiguration::kAllWorlds}
  };
  for (const auto& config : kIsFramePendingConfigurations) {
