...
static void BindInterfaceMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!request_handle) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(1, "MojoHandle"));
    return;
  }
  ...
  if (!info[2]->IsUndefined()) {
    ...
    if (!scope.Prepare())
      return;
    const char* const kValidScopeValues[] = {
        "context",
        "process",
    };
  ...
  if (!info[3]->IsUndefined()) {
    use_browser_interface_broker = NativeValueTraits<IDLBoolean>::NativeValue(info.GetIsolate(), info[3], exception_state);
    if (exception_state.HadException())
      return;
  } else {
    use_browser_interface_broker = false;
  }

  Mojo::bindInterface(script_state, interface_name, request_handle, scope, use_browser_interface_broker);
}
