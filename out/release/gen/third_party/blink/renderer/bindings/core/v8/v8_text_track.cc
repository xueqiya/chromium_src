...
static void AddCueMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!cue) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("addCue", "TextTrack", ExceptionMessages::ArgumentNotOfType(0, "TextTrackCue")));
    return;
  }
...
static void RemoveCueMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  cue = V8TextTrackCue::ToImplWithTypeCheck(info.GetIsolate(), info[0]);
  if (!cue) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(0, "TextTrackCue"));
    return;
  }
