...
static void WriteMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (UNLIKELY(num_args_passed <= 0)) {
    ScriptPromise result = impl->write(script_state, exception_state);
    if (exception_state.HadException()) {
      return;
    }
    V8SetReturnValue(info, result.V8Value());
  ...
  ScriptPromise result = impl->write(script_state, chunk, exception_state);
  if (exception_state.HadException()) {
    return;
  }
  V8SetReturnValue(info, result.V8Value());
...
static void Constructor(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!stream) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(0, "WritableStream"));
    return;
  }
