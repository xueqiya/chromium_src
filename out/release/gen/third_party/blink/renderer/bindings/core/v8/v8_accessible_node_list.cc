...
static void AddMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!node) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("add", "AccessibleNodeList", ExceptionMessages::ArgumentNotOfType(0, "AccessibleNode")));
    return;
  }
  ...
  if (!info[1]->IsUndefined()) {
    ...
    if (!before && !IsUndefinedOrNull(info[1])) {
      V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("add", "AccessibleNodeList", ExceptionMessages::ArgumentNotOfType(1, "AccessibleNode")));
      return;
    }
  } else {
    before = nullptr;
