...
static void InitializeMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!new_item) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(0, "SVGTransform"));
    return;
  }
...
static void CreateSVGTransformFromMatrixMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!matrix) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("createSVGTransformFromMatrix", "SVGTransformList", ExceptionMessages::ArgumentNotOfType(0, "SVGMatrix")));
    return;
  }
