...
static void SetOrientToAngleMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!angle) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("setOrientToAngle", "SVGMarkerElement", ExceptionMessages::ArgumentNotOfType(0, "SVGAngle")));
    return;
  }
