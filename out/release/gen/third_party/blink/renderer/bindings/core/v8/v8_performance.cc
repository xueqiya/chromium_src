...
static void MeasureMemoryMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  options = NativeValueTraits<MeasureMemoryOptions>::NativeValue(info.GetIsolate(), info[0], exception_state);
  if (exception_state.HadException())
    return;

  ScriptPromise result = impl->measureMemory(script_state, options, exception_state);
  if (exception_state.HadException()) {
    return;
  }
...
void V8Performance::InstallConditionalFeatures(
  ...
  if (!prototype_object.IsEmpty() || !interface_object.IsEmpty()) {
    ...
    if (execution_context && (execution_context->IsDocument() || execution_context->IsWorkerGlobalScope())) {
      if (RuntimeEnabledFeatures::MeasureMemoryEnabled()) {
        {
          // Install measureMemory configuration
