...
static void Constructor(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!x) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(0, "CSSNumericValue"));
    return;
  }

  y = V8CSSNumericValue::ToImplWithTypeCheck(info.GetIsolate(), info[1]);
  if (!y) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(1, "CSSNumericValue"));
    return;
  }
  ...
  if (!z) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(2, "CSSNumericValue"));
    return;
  }
