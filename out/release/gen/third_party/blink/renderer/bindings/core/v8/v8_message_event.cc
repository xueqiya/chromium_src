...
static void PortsAttributeGetter(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  // [CachedAttribute]
  static const V8PrivateProperty::SymbolKey kPrivatePropertyPorts;

  V8PrivateProperty::Symbol property_symbol =
      V8PrivateProperty::GetSymbol(info.GetIsolate(),
          kPrivatePropertyPorts);
...
static void InitMessageEventMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!info[3]->IsUndefined()) {
    data = ScriptValue(info.GetIsolate(), info[3]);
  } else {
    data = ScriptValue::CreateNull(script_state->GetIsolate());
  }
  ...
  if (!info[6]->IsUndefined()) {
    ...
    if (!source && !IsUndefinedOrNull(info[6])) {
      exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(6, "EventTarget"));
      return;
    }
  } else {
    source = nullptr;
