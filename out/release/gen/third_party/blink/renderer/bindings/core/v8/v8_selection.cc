...
static void AddRangeMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!range) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("addRange", "Selection", ExceptionMessages::ArgumentNotOfType(0, "Range")));
    return;
  }
...
static void RemoveRangeMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  range = V8Range::ToImplWithTypeCheck(info.GetIsolate(), info[0]);
  if (!range) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("removeRange", "Selection", ExceptionMessages::ArgumentNotOfType(0, "Range")));
    return;
  }
...
static void SetBaseAndExtentMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!extent_node && !IsUndefinedOrNull(info[2])) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(2, "Node"));
    return;
  }
