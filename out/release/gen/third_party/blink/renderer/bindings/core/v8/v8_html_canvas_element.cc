...
static void ToDataURLMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  quality = ScriptValue(info.GetIsolate(), info[1]);

  String result = impl->toDataURL(type, quality, exception_state);
  if (exception_state.HadException()) {
    return;
  }
  V8SetReturnValueString(info, result, info.GetIsolate());
...
static void ToBlobMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  quality = ScriptValue(info.GetIsolate(), info[2]);

  impl->toBlob(callback, type, quality, exception_state);
  if (exception_state.HadException()) {
    return;
  }
}
