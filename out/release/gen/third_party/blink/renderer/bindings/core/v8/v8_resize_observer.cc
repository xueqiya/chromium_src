...
static void ObserveMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!target) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("observe", "ResizeObserver", ExceptionMessages::ArgumentNotOfType(0, "Element")));
    return;
  }
...
static void UnobserveMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  target = V8Element::ToImplWithTypeCheck(info.GetIsolate(), info[0]);
  if (!target) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("unobserve", "ResizeObserver", ExceptionMessages::ArgumentNotOfType(0, "Element")));
    return;
  }
