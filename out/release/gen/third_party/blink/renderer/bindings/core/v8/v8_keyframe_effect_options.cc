...
void V8KeyframeEffectOptions::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, KeyframeEffectOptions* impl, ExceptionState& exception_state) {
  ...
  if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&pseudo_element_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  if (pseudo_element_value.IsEmpty() || pseudo_element_value->IsUndefined()) {
    ...
    V8StringResource<kTreatNullAndUndefinedAsNullString> pseudo_element_cpp_value = pseudo_element_value;
...
bool toV8KeyframeEffectOptions(const KeyframeEffectOptions* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (impl->hasPseudoElement()) {
    ...
    pseudo_element_value = v8::Null(isolate);
    pseudo_element_has_value_or_default = true;
  }
  if (pseudo_element_has_value_or_default &&
      !create_property(1, pseudo_element_value)) {
    return false;
  }
...
KeyframeEffectOptions* NativeValueTraits<KeyframeEffectOptions>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  KeyframeEffectOptions* impl = MakeGarbageCollected<KeyframeEffectOptions>();
