...
static void RegisterPropertyMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!info[0]->IsNullOrUndefined() && !info[0]->IsObject()) {
    exception_state.ThrowTypeError("parameter 1 ('definition') is not an object.");
    return;
  }
  definition = NativeValueTraits<PropertyDefinition>::NativeValue(info.GetIsolate(), info[0], exception_state);
  if (exception_state.HadException())
    return;

  ExecutionContext* execution_context = ExecutionContext::ForCurrentRealm(info);
  PropertyRegistration::registerProperty(execution_context, definition, exception_state);
  if (exception_state.HadException()) {
    return;
  }
}
