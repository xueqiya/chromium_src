...
static void Constructor(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!info[2]->IsUndefined()) {
    use_browser_interface_broker = NativeValueTraits<IDLBoolean>::NativeValue(info.GetIsolate(), info[2], exception_state);
    if (exception_state.HadException())
      return;
  } else {
  ...
  MojoInterfaceInterceptor* impl = MojoInterfaceInterceptor::Create(execution_context, interface_name, scope, use_browser_interface_broker, exception_state);
  if (exception_state.HadException()) {
    return;
  }
