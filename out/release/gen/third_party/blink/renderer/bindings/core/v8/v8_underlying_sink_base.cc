...
static void StartMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  controller = ScriptValue(info.GetIsolate(), info[0]);

  ScriptPromise result = impl->start(script_state, controller, exception_state);
...
static void WriteMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  ScriptValue chunk;
  ScriptValue controller;
  chunk = ScriptValue(info.GetIsolate(), info[0]);

  controller = ScriptValue(info.GetIsolate(), info[1]);

  ScriptPromise result = impl->write(script_state, chunk, controller, exception_state);
  if (exception_state.HadException()) {
    return;
  }
  V8SetReturnValue(info, result.V8Value());
