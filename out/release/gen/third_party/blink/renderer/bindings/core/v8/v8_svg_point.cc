...
static void MatrixTransformMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!matrix) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("matrixTransform", "SVGPoint", ExceptionMessages::ArgumentNotOfType(0, "SVGMatrix")));
    return;
  }
