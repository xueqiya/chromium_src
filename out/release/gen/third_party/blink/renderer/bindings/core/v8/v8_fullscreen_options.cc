...
void V8FullscreenOptions::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, FullscreenOptions* impl, ExceptionState& exception_state) {
  ...
  if (RuntimeEnabledFeatures::WindowPlacementEnabled()) {
    v8::Local<v8::Value> screen_value;
    if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&screen_value)) {
      exception_state.RethrowV8Exception(block.Exception());
      return;
    }
    if (screen_value.IsEmpty() || screen_value->IsUndefined()) {
      // Do nothing.
    } else if (screen_value->IsNull()) {
      impl->setScreenToNull();
    } else {
      Screen* screen_cpp_value = V8Screen::ToImplWithTypeCheck(isolate, screen_value);
      if (!screen_cpp_value) {
        exception_state.ThrowTypeError("member screen is not of type Screen.");
        return;
      }
      impl->setScreen(screen_cpp_value);
    }
  }
}
...
bool toV8FullscreenOptions(const FullscreenOptions* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (RuntimeEnabledFeatures::WindowPlacementEnabled()) {
    v8::Local<v8::Value> screen_value;
    bool screen_has_value_or_default = false;
    if (impl->hasScreen()) {
      screen_value = ToV8(impl->screen(), creationContext, isolate);
      screen_has_value_or_default = true;
    }
    if (screen_has_value_or_default &&
        !create_property(1, screen_value)) {
      return false;
    }
  }
...
FullscreenOptions* NativeValueTraits<FullscreenOptions>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  FullscreenOptions* impl = MakeGarbageCollected<FullscreenOptions>();
