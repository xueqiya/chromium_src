...
static void SelectFontsFromOwnerDocumentMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!target_document) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("selectFontsFromOwnerDocument", "PagePopupController", ExceptionMessages::ArgumentNotOfType(0, "Document")));
    return;
  }
...
void V8PagePopupController::SelectFontsFromOwnerDocumentMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  BLINK_BINDINGS_TRACE_EVENT("PagePopupController.selectFontsFromOwnerDocument");
