...
static void Constructor(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  StringOrWorkerOptions options;
  script_url = info[0];
  if (!script_url.Prepare())
    return;

  if (!info[1]->IsUndefined()) {
    V8StringOrWorkerOptions::ToImpl(info.GetIsolate(), info[1], options, UnionTypeConversionMode::kNotNullable, exception_state);
    if (exception_state.HadException())
      return;
  } else {
    options.SetWorkerOptions(MakeGarbageCollected<WorkerOptions>());
  }

  ExecutionContext* execution_context = ToExecutionContext(
      info.NewTarget().As<v8::Object>()->CreationContext());
  SharedWorker* impl = SharedWorker::Create(execution_context, script_url, options, exception_state);
  if (exception_state.HadException()) {
    return;
  }
