...
static_assert(
    ...
    "UnderlyingSourceBase inherits from ActiveScriptWrappable<>, but is not specifying "
    "[ActiveScriptWrappable] extended attribute in the IDL file.  "
    "Be consistent.");
static_assert(
    std::is_same<decltype(&UnderlyingSourceBase::HasPendingActivity),
                 decltype(&ScriptWrappable::HasPendingActivity)>::value,
    "UnderlyingSourceBase is overriding hasPendingActivity(), but is not specifying "
    "[ActiveScriptWrappable] extended attribute in the IDL file.  "
    "Be consistent.");
...
static void StartMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  ScriptValue stream;
  stream = ScriptValue(info.GetIsolate(), info[0]);
