...
void V8FragmentResultOptions::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, FragmentResultOptions* impl, ExceptionState& exception_state) {
  ...
  v8::Local<v8::Value> baseline_value;
  if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&baseline_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  if (baseline_value.IsEmpty() || baseline_value->IsUndefined()) {
    // Do nothing.
  } else {
    double baseline_cpp_value = NativeValueTraits<IDLDouble>::NativeValue(isolate, baseline_value, exception_state);
    if (exception_state.HadException())
      return;
    impl->setBaseline(baseline_cpp_value);
  }

  v8::Local<v8::Value> child_fragments_value;
  if (!v8Object->Get(context, keys[2].Get(isolate)).ToLocal(&child_fragments_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (data_value.IsEmpty() || data_value->IsUndefined()) {
    ...
    ScriptValue data_cpp_value = ScriptValue(isolate, data_value);
...
bool toV8FragmentResultOptions(const FragmentResultOptions* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  v8::Local<v8::Value> baseline_value;
  bool baseline_has_value_or_default = false;
  if (impl->hasBaseline()) {
    baseline_value = v8::Number::New(isolate, impl->baseline());
    baseline_has_value_or_default = true;
  }
  if (baseline_has_value_or_default &&
      !create_property(1, baseline_value)) {
    return false;
  }
  ...
  if (child_fragments_has_value_or_default &&
      !create_property(2, child_fragments_value)) {
    return false;
  }
...
FragmentResultOptions* NativeValueTraits<FragmentResultOptions>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  FragmentResultOptions* impl = MakeGarbageCollected<FragmentResultOptions>();
