...
static void CreateNSResolverMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!node_resolver) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("createNSResolver", "XPathEvaluator", ExceptionMessages::ArgumentNotOfType(0, "Node")));
    return;
  }
