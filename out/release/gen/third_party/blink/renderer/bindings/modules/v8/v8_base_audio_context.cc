...
static void CreatePeriodicWaveMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!info[2]->IsNullOrUndefined() && !info[2]->IsObject()) {
    exception_state.ThrowTypeError("parameter 3 ('constraints') is not an object.");
    return;
  }
  constraints = NativeValueTraits<PeriodicWaveConstraints>::NativeValue(info.GetIsolate(), info[2], exception_state);
  if (exception_state.HadException())
    return;

  PeriodicWave* result = impl->createPeriodicWave(real, imag, constraints, exception_state);
  if (exception_state.HadException()) {
    return;
  }
  V8SetReturnValue(info, result);
