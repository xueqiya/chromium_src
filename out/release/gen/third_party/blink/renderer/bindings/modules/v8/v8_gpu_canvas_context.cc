...
static void GetSwapChainPreferredFormatMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ExceptionState exception_state(info.GetIsolate(), ExceptionState::kExecutionContext, "GPUCanvasContext", "getSwapChainPreferredFormat");
  ExceptionToRejectPromiseScope reject_promise_scope(info, exception_state);

  // V8DOMConfiguration::kDoNotCheckHolder
  // Make sure that info.Holder() really points to an instance of the type.
  if (!V8GPUCanvasContext::HasInstance(info.Holder(), info.GetIsolate())) {
  ...
  GPUDevice* device;
  device = V8GPUDevice::ToImplWithTypeCheck(info.GetIsolate(), info[0]);
  if (!device) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(0, "GPUDevice"));
    return;
  }

  ScriptPromise result = impl->getSwapChainPreferredFormat(script_state, device);
...
void V8GPUCanvasContext::ConfigureSwapChainMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  BLINK_BINDINGS_TRACE_EVENT("GPUCanvasContext.configureSwapChain");
...
void V8GPUCanvasContext::GetSwapChainPreferredFormatMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  BLINK_BINDINGS_TRACE_EVENT("GPUCanvasContext.getSwapChainPreferredFormat");
  RUNTIME_CALL_TIMER_SCOPE_DISABLED_BY_DEFAULT(info.GetIsolate(), "Blink_GPUCanvasContext_getSwapChainPreferredFormat");

  gpu_canvas_context_v8_internal::GetSwapChainPreferredFormatMethod(info);
}
...
static constexpr V8DOMConfiguration::MethodConfiguration kV8GPUCanvasContextMethods[] = {
    ...
    {"getSwapChainPreferredFormat", V8GPUCanvasContext::GetSwapChainPreferredFormatMethodCallback, 1, v8::None, V8DOMConfiguration::kOnPrototype, V8DOMConfiguration::kDoNotCheckHolder, V8DOMConfiguration::kDoNotCheckAccess, V8DOMConfiguration::kHasSideEffect, V8DOMConfiguration::kAllWorlds},
};
