...
void V8AudioConfiguration::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, AudioConfiguration* impl, ExceptionState& exception_state) {
  ...
  if (RuntimeEnabledFeatures::MediaCapabilitiesSpatialAudioEnabled()) {
    v8::Local<v8::Value> spatial_rendering_value;
    if (!v8Object->Get(context, keys[4].Get(isolate)).ToLocal(&spatial_rendering_value)) {
      exception_state.RethrowV8Exception(block.Exception());
      return;
    }
    if (spatial_rendering_value.IsEmpty() || spatial_rendering_value->IsUndefined()) {
      // Do nothing.
    } else {
      bool spatial_rendering_cpp_value = NativeValueTraits<IDLBoolean>::NativeValue(isolate, spatial_rendering_value, exception_state);
      if (exception_state.HadException())
        return;
      impl->setSpatialRendering(spatial_rendering_cpp_value);
    }
  }
}
...
bool toV8AudioConfiguration(const AudioConfiguration* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (RuntimeEnabledFeatures::MediaCapabilitiesSpatialAudioEnabled()) {
    v8::Local<v8::Value> spatial_rendering_value;
    bool spatial_rendering_has_value_or_default = false;
    if (impl->hasSpatialRendering()) {
      spatial_rendering_value = v8::Boolean::New(isolate, impl->spatialRendering());
      spatial_rendering_has_value_or_default = true;
    }
    if (spatial_rendering_has_value_or_default &&
        !create_property(4, spatial_rendering_value)) {
      return false;
    }
  }
...
AudioConfiguration* NativeValueTraits<AudioConfiguration>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  AudioConfiguration* impl = MakeGarbageCollected<AudioConfiguration>();
