...
static void CopyBufferToTextureMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  V8UnsignedLongSequenceOrGPUExtent3DDict::ToImpl(info.GetIsolate(), info[2], copy_size, UnionTypeConversionMode::kNotNullable, exception_state);
  if (exception_state.HadException())
    return;
...
static void PushDebugGroupMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (UNLIKELY(info.Length() < 1)) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("pushDebugGroup", "GPUCommandEncoder", ExceptionMessages::NotEnoughArguments(1, info.Length())));
    return;
  }
...
static void InsertDebugMarkerMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (UNLIKELY(info.Length() < 1)) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("insertDebugMarker", "GPUCommandEncoder", ExceptionMessages::NotEnoughArguments(1, info.Length())));
    return;
  }
