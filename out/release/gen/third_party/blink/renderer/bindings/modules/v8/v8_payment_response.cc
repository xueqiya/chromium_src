...
static void CompleteMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!info[0]->IsUndefined()) {
    ...
    if (!payment_result.Prepare(exception_state))
      return;
    const char* const kValidPaymentResultValues[] = {
        "fail",
        "success",
        "unknown",
    };
  ...
  ScriptPromise result = impl->complete(script_state, payment_result, exception_state);
...
static void RetryMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  ScriptPromise result = impl->retry(script_state, error_fields, exception_state);
  if (exception_state.HadException()) {
    return;
  }
  V8SetReturnValue(info, result.V8Value());
