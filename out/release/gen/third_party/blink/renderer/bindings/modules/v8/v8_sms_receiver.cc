...
static_assert(
    ...
    "SMSReceiver inherits from ActiveScriptWrappable<>, but is not specifying "
    "[ActiveScriptWrappable] extended attribute in the IDL file.  "
    "Be consistent.");
static_assert(
    std::is_same<decltype(&SMSReceiver::HasPendingActivity),
                 decltype(&ScriptWrappable::HasPendingActivity)>::value,
    "SMSReceiver is overriding hasPendingActivity(), but is not specifying "
    "[ActiveScriptWrappable] extended attribute in the IDL file.  "
    "Be consistent.");

namespace sms_receiver_v8_internal {

static void ReceiveMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ExceptionState exception_state(info.GetIsolate(), ExceptionState::kExecutionContext, "SMSReceiver", "receive");
  ...
  ScriptPromise result = impl->receive(script_state, options, exception_state);
  if (exception_state.HadException()) {
    return;
  }
...
void V8SMSReceiver::ReceiveMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  BLINK_BINDINGS_TRACE_EVENT("SMSReceiver.receive");
  RUNTIME_CALL_TIMER_SCOPE_DISABLED_BY_DEFAULT(info.GetIsolate(), "Blink_SMSReceiver_receive");

  ExecutionContext* execution_context_for_measurement = CurrentExecutionContext(info.GetIsolate());
  UseCounter::Count(execution_context_for_measurement, WebFeature::kSMSReceiverStart);
  sms_receiver_v8_internal::ReceiveMethod(info);
}
...
static void InstallV8SMSReceiverTemplate(
  ...
  V8DOMConfiguration::InitializeDOMInterfaceTemplate(isolate, interface_template, V8SMSReceiver::GetWrapperTypeInfo()->interface_name, v8::Local<v8::FunctionTemplate>(), V8SMSReceiver::kInternalFieldCount);
...
void V8SMSReceiver::InstallConditionalFeatures(
  ...
  if (!prototype_object.IsEmpty() || !interface_object.IsEmpty()) {
    if (is_secure_context) {
      {
        // Install receive configuration
        const V8DOMConfiguration::MethodConfiguration kConfigurations[] = {
            {"receive", V8SMSReceiver::ReceiveMethodCallback, 0, v8::None, V8DOMConfiguration::kOnPrototype, V8DOMConfiguration::kDoNotCheckHolder, V8DOMConfiguration::kDoNotCheckAccess, V8DOMConfiguration::kHasSideEffect, V8DOMConfiguration::kAllWorlds}
        };
