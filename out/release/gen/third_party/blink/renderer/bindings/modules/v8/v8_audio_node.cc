...
static void Connect1Method(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!destination) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(0, "AudioNode"));
    return;
  }
...
static void Connect2Method(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!destination) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(0, "AudioParam"));
    return;
  }
