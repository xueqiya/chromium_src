...
void V8GPUTextureDescriptor::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, GPUTextureDescriptor* impl, ExceptionState& exception_state) {
  if (IsUndefinedOrNull(v8_value)) {
    exception_state.ThrowTypeError("Missing required member(s): format, size, usage.");
    return;
  }
  ...
  if (size_value.IsEmpty() || size_value->IsUndefined()) {
    ...
    UnsignedLongSequenceOrGPUExtent3DDict size_cpp_value;
    V8UnsignedLongSequenceOrGPUExtent3DDict::ToImpl(isolate, size_value, size_cpp_value, UnionTypeConversionMode::kNotNullable, exception_state);
    if (exception_state.HadException())
      return;
...
GPUTextureDescriptor* NativeValueTraits<GPUTextureDescriptor>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  GPUTextureDescriptor* impl = MakeGarbageCollected<GPUTextureDescriptor>();
