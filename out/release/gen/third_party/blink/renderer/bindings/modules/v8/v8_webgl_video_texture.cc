...
static void VideoElementTargetVideoTextureMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!video) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(1, "HTMLVideoElement"));
    return;
  }

  ExecutionContext* execution_context = ExecutionContext::ForRelevantRealm(info);
  VideoFrameMetadata* result = impl->VideoElementTargetVideoTexture(execution_context, target, video, exception_state);
  if (exception_state.HadException()) {
    return;
  }
...
void V8WebGLVideoTexture::VideoElementTargetVideoTextureMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  BLINK_BINDINGS_TRACE_EVENT("WebGLVideoTexture.VideoElementTargetVideoTexture");
