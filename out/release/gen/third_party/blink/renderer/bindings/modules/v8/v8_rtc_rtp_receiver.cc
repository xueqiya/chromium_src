...
static void PlayoutDelayHintAttributeGetter(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  ExceptionState exception_state(info.GetIsolate(), ExceptionState::kGetterContext, "RTCRtpReceiver", "playoutDelayHint");
  bool is_null = false;

  double cpp_value(impl->playoutDelayHint(is_null, exception_state));
...
static void PlayoutDelayHintAttributeSetter(
  ...
  impl->setPlayoutDelayHint(cpp_value, is_null, exception_state);
}
