...
static void Transaction2Method(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  options = NativeValueTraits<IDBTransactionOptions>::NativeValue(info.GetIsolate(), info[2], exception_state);
  if (exception_state.HadException())
    return;

  IDBTransaction* result = impl->transaction(script_state, store_names, mode, options, exception_state);
  if (exception_state.HadException()) {
    return;
  }
  // [NewObject] must always create a new wrapper.  Check that a wrapper
...
static int TransactionMethodMaxArg() {
...
static void TransactionMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  switch (std::min(idb_database_v8_internal::TransactionMethodMaxArg(), info.Length())) {
    case 1:
      if (true) {
...
void V8IDBDatabase::CreateObjectStoreMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  UseCounter::Count(execution_context_for_measurement, WebFeature::kIndexedDBWrite);
