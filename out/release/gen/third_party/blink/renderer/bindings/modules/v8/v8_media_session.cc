...
static void SetActionHandlerMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!action.Prepare())
    return;
  const char* const kValidActionValues[] = {
      "play",
      "pause",
      "previoustrack",
      "nexttrack",
...
static void SetPositionStateMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ExceptionState exception_state(info.GetIsolate(), ExceptionState::kExecutionContext, "MediaSession", "setPositionState");
  ...
  MediaPositionState* state;
  if (!info[0]->IsNullOrUndefined() && !info[0]->IsObject()) {
    exception_state.ThrowTypeError("parameter 1 ('state') is not an object.");
    return;
  }
  state = NativeValueTraits<MediaPositionState>::NativeValue(info.GetIsolate(), info[0], exception_state);
  if (exception_state.HadException())
    return;

  impl->setPositionState(state, exception_state);
  if (exception_state.HadException()) {
    return;
  }
}
...
void V8MediaSession::SetPositionStateMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  ExecutionContext* execution_context_for_measurement = CurrentExecutionContext(info.GetIsolate());
  UseCounter::Count(execution_context_for_measurement, WebFeature::kV8MediaSession_SetPositionState_Method);
  media_session_v8_internal::SetPositionStateMethod(info);
}
...
void V8MediaSession::InstallRuntimeEnabledFeaturesOnTemplate(
  ...
  // Custom signature
  if (RuntimeEnabledFeatures::MediaSessionPositionEnabled()) {
    {
      // Install setPositionState configuration
      constexpr V8DOMConfiguration::MethodConfiguration kConfigurations[] = {
          {"setPositionState", V8MediaSession::SetPositionStateMethodCallback, 1, v8::None, V8DOMConfiguration::kOnPrototype, V8DOMConfiguration::kCheckHolder, V8DOMConfiguration::kDoNotCheckAccess, V8DOMConfiguration::kHasSideEffect, V8DOMConfiguration::kAllWorlds}
      };
