...
static const v8::Eternal<v8::Name>* eternalV8ContentDescriptionKeys(v8::Isolate* isolate) {
...
void V8ContentDescription::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, ContentDescription* impl, ExceptionState& exception_state) {
  if (IsUndefinedOrNull(v8_value)) {
    exception_state.ThrowTypeError("Missing required member(s): description, id, launchUrl, title.");
    return;
  }
  ...
  const v8::Eternal<v8::Name>* keys = eternalV8ContentDescriptionKeys(isolate);
  ...
  v8::Local<v8::Value> launch_url_value;
  if (!v8Object->Get(context, keys[4].Get(isolate)).ToLocal(&launch_url_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  if (launch_url_value.IsEmpty() || launch_url_value->IsUndefined()) {
    exception_state.ThrowTypeError("required member launchUrl is undefined.");
    return;
  } else {
    V8StringResource<> launch_url_cpp_value = NativeValueTraits<IDLUSVString>::NativeValue(isolate, launch_url_value, exception_state);
    if (exception_state.HadException())
      return;
    impl->setLaunchUrl(launch_url_cpp_value);
  }

  v8::Local<v8::Value> title_value;
  if (!v8Object->Get(context, keys[5].Get(isolate)).ToLocal(&title_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
...
bool toV8ContentDescription(const ContentDescription* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  v8::Local<v8::Value> launch_url_value;
  bool launch_url_has_value_or_default = false;
  if (impl->hasLaunchUrl()) {
    launch_url_value = V8String(isolate, impl->launchUrl());
    launch_url_has_value_or_default = true;
  } else {
    NOTREACHED();
  }
  if (launch_url_has_value_or_default &&
      !create_property(4, launch_url_value)) {
    return false;
  }
