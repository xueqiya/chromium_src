...
const WrapperTypeInfo v8_file_system_file_handle_wrapper_type_info = {
    ...
    V8FileSystemFileHandle::InstallConditionalFeatures,
...
static void CreateWriterMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  FileSystemCreateWriterOptions* options;
  if (!info[0]->IsNullOrUndefined() && !info[0]->IsObject()) {
    exception_state.ThrowTypeError("parameter 1 ('options') is not an object.");
    return;
  }
  options = NativeValueTraits<FileSystemCreateWriterOptions>::NativeValue(info.GetIsolate(), info[0], exception_state);
  if (exception_state.HadException())
    return;

  ScriptPromise result = impl->createWriter(script_state, options);
  V8SetReturnValue(info, result.V8Value());
}

static void CreateWritableMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ExceptionState exception_state(info.GetIsolate(), ExceptionState::kExecutionContext, "FileSystemFileHandle", "createWritable");
  ExceptionToRejectPromiseScope reject_promise_scope(info, exception_state);
  ...
  ScriptPromise result = impl->createWritable(script_state, options, exception_state);
  if (exception_state.HadException()) {
    return;
  }
  V8SetReturnValue(info, result.V8Value());
...
static void GetFileMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  ScriptPromise result = impl->getFile(script_state, exception_state);
  if (exception_state.HadException()) {
    return;
  }
  V8SetReturnValue(info, result.V8Value());
...
void V8FileSystemFileHandle::CreateWriterMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  BLINK_BINDINGS_TRACE_EVENT("FileSystemFileHandle.createWriter");
...
void V8FileSystemFileHandle::CreateWritableMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  BLINK_BINDINGS_TRACE_EVENT("FileSystemFileHandle.createWritable");
  RUNTIME_CALL_TIMER_SCOPE_DISABLED_BY_DEFAULT(info.GetIsolate(), "Blink_NativeFileSystemFileHandle_createWritable");

  native_file_system_file_handle_v8_internal::CreateWritableMethod(info);
}
...
void V8FileSystemFileHandle::InstallConditionalFeatures(
  ...
  if (!prototype_object.IsEmpty() || !interface_object.IsEmpty()) {
    if (is_secure_context) {
      {
        // Install createWriter configuration
    ...
    if (is_secure_context) {
      if (RuntimeEnabledFeatures::WritableFileStreamEnabled()) {
        {
          // Install createWritable configuration
          const V8DOMConfiguration::MethodConfiguration kConfigurations[] = {
              {"createWritable", V8FileSystemFileHandle::CreateWritableMethodCallback, 0, v8::None, V8DOMConfiguration::kOnPrototype, V8DOMConfiguration::kDoNotCheckHolder, V8DOMConfiguration::kDoNotCheckAccess, V8DOMConfiguration::kHasSideEffect, V8DOMConfiguration::kAllWorlds}
          };
          for (const auto& config : kConfigurations) {
            V8DOMConfiguration::InstallMethod(
                isolate, world, instance_object, prototype_object,
