...
static void AddPerformanceEntry1Method(const v8::FunctionCallbackInfo<v8::Value>& info) {
  FetchEvent* impl = V8FetchEvent::ToImpl(info.Holder());

  PerformanceMark* entry;
  entry = V8PerformanceMark::ToImplWithTypeCheck(info.GetIsolate(), info[0]);
  if (!entry) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("addPerformanceEntry", "FetchEvent", ExceptionMessages::ArgumentNotOfType(0, "PerformanceMark")));
    return;
  }

  impl->addPerformanceEntry(entry);
}

static void AddPerformanceEntry2Method(const v8::FunctionCallbackInfo<v8::Value>& info) {
  FetchEvent* impl = V8FetchEvent::ToImpl(info.Holder());

  PerformanceMeasure* entry;
  entry = V8PerformanceMeasure::ToImplWithTypeCheck(info.GetIsolate(), info[0]);
  if (!entry) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("addPerformanceEntry", "FetchEvent", ExceptionMessages::ArgumentNotOfType(0, "PerformanceMeasure")));
    return;
  }
...
static void AddPerformanceEntryMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  switch (std::min(1, info.Length())) {
    case 1:
      if (V8PerformanceMark::HasInstance(info[0], info.GetIsolate())) {
        AddPerformanceEntry1Method(info);
        return;
      }
      if (V8PerformanceMeasure::HasInstance(info[0], info.GetIsolate())) {
        AddPerformanceEntry2Method(info);
        return;
      }
      break;
    default:
  ...
  ExceptionState exception_state(info.GetIsolate(), ExceptionState::kExecutionContext, "FetchEvent", "addPerformanceEntry");
  if (is_arity_error) {
    if (info.Length() < 1) {
      exception_state.ThrowTypeError(ExceptionMessages::NotEnoughArguments(1, info.Length()));
      return;
...
void V8FetchEvent::AddPerformanceEntryMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  BLINK_BINDINGS_TRACE_EVENT("FetchEvent.addPerformanceEntry");
  RUNTIME_CALL_TIMER_SCOPE_DISABLED_BY_DEFAULT(info.GetIsolate(), "Blink_FetchEvent_addPerformanceEntry");

  fetch_event_v8_internal::AddPerformanceEntryMethod(info);
}
...
void V8FetchEvent::InstallRuntimeEnabledFeaturesOnTemplate(
  ...
  // Custom signature
  if (RuntimeEnabledFeatures::ServiceWorkerFetchEventWorkerTimingEnabled()) {
    {
      // Install addPerformanceEntry configuration
      constexpr V8DOMConfiguration::MethodConfiguration kConfigurations[] = {
          {"addPerformanceEntry", V8FetchEvent::AddPerformanceEntryMethodCallback, 1, v8::None, V8DOMConfiguration::kOnPrototype, V8DOMConfiguration::kCheckHolder, V8DOMConfiguration::kDoNotCheckAccess, V8DOMConfiguration::kHasSideEffect, V8DOMConfiguration::kAllWorlds}
      };
