...
static void CopyFromChannelMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!info[2]->IsUndefined()) {
    buffer_offset = NativeValueTraits<IDLUnsignedLong>::NativeValue(info.GetIsolate(), info[2], exception_state);
    if (exception_state.HadException())
      return;
  } else {
    buffer_offset = 0u;
  ...
  impl->copyFromChannel(destination, channel_number, buffer_offset, exception_state);
  if (exception_state.HadException()) {
    return;
  }
}
...
static void CopyToChannelMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  impl->copyToChannel(source, channel_number, buffer_offset, exception_state);
  if (exception_state.HadException()) {
    return;
  }
}
