...
void V8RTCRtpContributingSource::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, RTCRtpContributingSource* impl, ExceptionState& exception_state) {
  if (IsUndefinedOrNull(v8_value)) {
    exception_state.ThrowTypeError("Missing required member(s): rtpTimestamp, source, timestamp.");
    return;
  }
  ...
  if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&rtp_timestamp_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  if (rtp_timestamp_value.IsEmpty() || rtp_timestamp_value->IsUndefined()) {
    exception_state.ThrowTypeError("required member rtpTimestamp is undefined.");
    return;
  } else {
  ...
  if (!v8Object->Get(context, keys[3].Get(isolate)).ToLocal(&timestamp_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
...
bool toV8RTCRtpContributingSource(const RTCRtpContributingSource* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (rtp_timestamp_has_value_or_default &&
      !create_property(1, rtp_timestamp_value)) {
    return false;
  }
  ...
  if (timestamp_has_value_or_default &&
      !create_property(3, timestamp_value)) {
    return false;
  }
...
RTCRtpContributingSource* NativeValueTraits<RTCRtpContributingSource>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  RTCRtpContributingSource* impl = MakeGarbageCollected<RTCRtpContributingSource>();
