...
void V8GPURenderPassColorAttachmentDescriptor::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, GPURenderPassColorAttachmentDescriptor* impl, ExceptionState& exception_state) {
  if (IsUndefinedOrNull(v8_value)) {
    exception_state.ThrowTypeError("Missing required member(s): attachment, loadValue.");
    return;
  }
  ...
  v8::Local<v8::Value> load_value_value;
  if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&load_value_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  if (load_value_value.IsEmpty() || load_value_value->IsUndefined()) {
    exception_state.ThrowTypeError("required member loadValue is undefined.");
    return;
  } else {
    GPULoadOpOrDoubleSequenceOrGPUColorDict load_value_cpp_value;
    V8GPULoadOpOrDoubleSequenceOrGPUColorDict::ToImpl(isolate, load_value_value, load_value_cpp_value, UnionTypeConversionMode::kNotNullable, exception_state);
    if (exception_state.HadException())
      return;
    impl->setLoadValue(load_value_cpp_value);
  }

  v8::Local<v8::Value> resolve_target_value;
  if (!v8Object->Get(context, keys[2].Get(isolate)).ToLocal(&resolve_target_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (!v8Object->Get(context, keys[3].Get(isolate)).ToLocal(&store_op_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
...
bool toV8GPURenderPassColorAttachmentDescriptor(const GPURenderPassColorAttachmentDescriptor* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  v8::Local<v8::Value> load_value_value;
  bool load_value_has_value_or_default = false;
  if (impl->hasLoadValue()) {
    load_value_value = ToV8(impl->loadValue(), creationContext, isolate);
    load_value_has_value_or_default = true;
  } else {
    NOTREACHED();
  }
  if (load_value_has_value_or_default &&
      !create_property(1, load_value_value)) {
    return false;
  }
  ...
  if (resolve_target_has_value_or_default &&
      !create_property(2, resolve_target_value)) {
    return false;
  }
  ...
  if (impl->hasStoreOp()) {
    ...
    store_op_value = V8String(isolate, "store");
    store_op_has_value_or_default = true;
  }
  if (store_op_has_value_or_default &&
      !create_property(3, store_op_value)) {
    return false;
  }
...
GPURenderPassColorAttachmentDescriptor* NativeValueTraits<GPURenderPassColorAttachmentDescriptor>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  GPURenderPassColorAttachmentDescriptor* impl = MakeGarbageCollected<GPURenderPassColorAttachmentDescriptor>();
