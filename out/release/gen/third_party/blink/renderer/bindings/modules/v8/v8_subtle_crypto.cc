...
static void EncryptMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!key) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(1, "CryptoKey"));
    return;
  }
...
static void WrapKeyMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!wrapping_key) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(2, "CryptoKey"));
    return;
  }
