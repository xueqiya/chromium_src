...
void V8RTCRtpCodecCapability::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, RTCRtpCodecCapability* impl, ExceptionState& exception_state) {
  ...
  if (RuntimeEnabledFeatures::RTCSvcScalabilityModeEnabled()) {
    v8::Local<v8::Value> scalability_modes_value;
    if (!v8Object->Get(context, keys[3].Get(isolate)).ToLocal(&scalability_modes_value)) {
      exception_state.RethrowV8Exception(block.Exception());
      return;
    }
    if (scalability_modes_value.IsEmpty() || scalability_modes_value->IsUndefined()) {
      // Do nothing.
    } else {
      Vector<String> scalability_modes_cpp_value = NativeValueTraits<IDLSequence<IDLString>>::NativeValue(isolate, scalability_modes_value, exception_state);
      if (exception_state.HadException())
        return;
      impl->setScalabilityModes(scalability_modes_cpp_value);
    }
  }
}
...
bool toV8RTCRtpCodecCapability(const RTCRtpCodecCapability* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (RuntimeEnabledFeatures::RTCSvcScalabilityModeEnabled()) {
    v8::Local<v8::Value> scalability_modes_value;
    bool scalability_modes_has_value_or_default = false;
    if (impl->hasScalabilityModes()) {
      scalability_modes_value = ToV8(impl->scalabilityModes(), creationContext, isolate);
      scalability_modes_has_value_or_default = true;
    }
    if (scalability_modes_has_value_or_default &&
        !create_property(3, scalability_modes_value)) {
      return false;
    }
  }
...
RTCRtpCodecCapability* NativeValueTraits<RTCRtpCodecCapability>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  RTCRtpCodecCapability* impl = MakeGarbageCollected<RTCRtpCodecCapability>();
