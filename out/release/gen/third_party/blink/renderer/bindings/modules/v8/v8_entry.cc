...
static void MoveToMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!parent) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("moveTo", "Entry", ExceptionMessages::ArgumentNotOfType(0, "DirectoryEntry")));
    return;
  }
...
static void CopyToMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  parent = V8DirectoryEntry::ToImplWithTypeCheck(info.GetIsolate(), info[0]);
  if (!parent) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("copyTo", "Entry", ExceptionMessages::ArgumentNotOfType(0, "DirectoryEntry")));
    return;
  }
