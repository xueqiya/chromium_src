...
static void SetPermissionMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  V8StringResource<> embedding_url;
  if (info[0]->IsObject()) {
    descriptor = ScriptValue(info.GetIsolate(), info[0]);
  } else {
  ...
  url = NativeValueTraits<IDLUSVString>::NativeValue(info.GetIsolate(), info[2], exception_state);
  if (exception_state.HadException())
    return;

  embedding_url = NativeValueTraits<IDLUSVString>::NativeValue(info.GetIsolate(), info[3], exception_state);
  if (exception_state.HadException())
    return;

  ScriptPromise result = InternalsPermission::setPermission(script_state, *impl, descriptor, state, url, embedding_url, exception_state);
  if (exception_state.HadException()) {
    return;
  }
  V8SetReturnValue(info, result.V8Value());
...
static constexpr V8DOMConfiguration::MethodConfiguration kV8InternalsMethods[] = {
    ...
    {"setPermission", V8InternalsPartial::SetPermissionMethodCallback, 4, v8::None, V8DOMConfiguration::kOnPrototype, V8DOMConfiguration::kDoNotCheckHolder, V8DOMConfiguration::kDoNotCheckAccess, V8DOMConfiguration::kHasSideEffect, V8DOMConfiguration::kAllWorlds},
