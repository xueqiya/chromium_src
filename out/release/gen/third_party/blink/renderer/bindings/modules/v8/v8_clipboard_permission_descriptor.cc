...
void V8ClipboardPermissionDescriptor::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, ClipboardPermissionDescriptor* impl, ExceptionState& exception_state) {
  ...
  v8::Local<v8::Value> allow_without_sanitization_value;
  if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&allow_without_sanitization_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  if (allow_without_sanitization_value.IsEmpty() || allow_without_sanitization_value->IsUndefined()) {
    // Do nothing.
  } else {
    bool allow_without_sanitization_cpp_value = NativeValueTraits<IDLBoolean>::NativeValue(isolate, allow_without_sanitization_value, exception_state);
    if (exception_state.HadException())
      return;
    impl->setAllowWithoutSanitization(allow_without_sanitization_cpp_value);
  }
}
...
bool toV8ClipboardPermissionDescriptor(const ClipboardPermissionDescriptor* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  v8::Local<v8::Value> allow_without_sanitization_value;
  bool allow_without_sanitization_has_value_or_default = false;
  if (impl->hasAllowWithoutSanitization()) {
    allow_without_sanitization_value = v8::Boolean::New(isolate, impl->allowWithoutSanitization());
    allow_without_sanitization_has_value_or_default = true;
  } else {
    allow_without_sanitization_value = v8::Boolean::New(isolate, false);
    allow_without_sanitization_has_value_or_default = true;
  }
  if (allow_without_sanitization_has_value_or_default &&
      !create_property(1, allow_without_sanitization_value)) {
    return false;
  }
...
ClipboardPermissionDescriptor* NativeValueTraits<ClipboardPermissionDescriptor>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  ClipboardPermissionDescriptor* impl = MakeGarbageCollected<ClipboardPermissionDescriptor>();
