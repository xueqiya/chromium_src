...
void V8ContactInfo::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, ContactInfo* impl, ExceptionState& exception_state) {
  ...
  if (address_value.IsEmpty() || address_value->IsUndefined()) {
    ...
    HeapVector<Member<ContactAddress>> address_cpp_value = NativeValueTraits<IDLSequence<ContactAddress>>::NativeValue(isolate, address_value, exception_state);
  ...
  if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&email_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (!v8Object->Get(context, keys[2].Get(isolate)).ToLocal(&icon_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  if (icon_value.IsEmpty() || icon_value->IsUndefined()) {
    ...
    HeapVector<Member<Blob>> icon_cpp_value = NativeValueTraits<IDLSequence<Blob>>::NativeValue(isolate, icon_value, exception_state);
    if (exception_state.HadException())
      return;
  ...
  if (!v8Object->Get(context, keys[4].Get(isolate)).ToLocal(&tel_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
...
bool toV8ContactInfo(const ContactInfo* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (impl->hasAddress()) {
    address_value = ToV8(impl->address(), creationContext, isolate);
  ...
  if (impl->hasIcon()) {
    icon_value = ToV8(impl->icon(), creationContext, isolate);
    icon_has_value_or_default = true;
  }
  if (icon_has_value_or_default &&
      !create_property(2, icon_value)) {
    return false;
  }
...
ContactInfo* NativeValueTraits<ContactInfo>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  ContactInfo* impl = MakeGarbageCollected<ContactInfo>();
