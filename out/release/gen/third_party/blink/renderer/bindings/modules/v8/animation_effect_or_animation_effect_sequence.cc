...
void V8AnimationEffectOrAnimationEffectSequence::ToImpl(
  ...
  if (v8_value->IsObject()) {
    ...
    if (!script_iterator.IsNull()) {
      HeapVector<Member<AnimationEffect>> cpp_value = NativeValueTraits<IDLSequence<AnimationEffect>>::NativeValue(isolate, std::move(script_iterator), exception_state);
      if (exception_state.HadException())
        return;
