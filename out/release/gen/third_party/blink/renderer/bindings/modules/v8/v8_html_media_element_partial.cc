...
static void SetMediaKeysMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!media_keys && !IsUndefinedOrNull(info[0])) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(0, "MediaKeys"));
    return;
  }

  ScriptPromise result = HTMLMediaElementEncryptedMedia::setMediaKeys(script_state, *impl, media_keys, exception_state);
  if (exception_state.HadException()) {
    return;
  }
  V8SetReturnValue(info, result.V8Value());
