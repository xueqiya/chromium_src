...
static void RawAttributeGetter(const v8::FunctionCallbackInfo<v8::Value>& info) {
  v8::Local<v8::Object> holder = info.Holder();

  ClipboardItem* impl = V8ClipboardItem::ToImpl(holder);

  V8SetReturnValueBool(info, impl->raw());
}
...
static void Constructor(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  options = NativeValueTraits<ClipboardItemOptions>::NativeValue(info.GetIsolate(), info[1], exception_state);
  if (exception_state.HadException())
    return;

  ClipboardItem* impl = ClipboardItem::Create(items, options, exception_state);
  if (exception_state.HadException()) {
    return;
  }
...
void V8ClipboardItem::RawAttributeGetterCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  RUNTIME_CALL_TIMER_SCOPE_DISABLED_BY_DEFAULT(info.GetIsolate(), "Blink_ClipboardItem_raw_Getter");

  clipboard_item_v8_internal::RawAttributeGetter(info);
}
...
void V8ClipboardItem::InstallRuntimeEnabledFeaturesOnTemplate(
  ...
  if (RuntimeEnabledFeatures::RawClipboardEnabled()) {
    static constexpr V8DOMConfiguration::AccessorConfiguration
    kAccessorConfigurations[] = {
        { "raw", V8ClipboardItem::RawAttributeGetterCallback, nullptr, V8PrivateProperty::kNoCachedAccessor, static_cast<v8::PropertyAttribute>(v8::ReadOnly), V8DOMConfiguration::kOnPrototype, V8DOMConfiguration::kCheckHolder, V8DOMConfiguration::kHasSideEffect, V8DOMConfiguration::kAlwaysCallGetter, V8DOMConfiguration::kAllWorlds },
    };
    V8DOMConfiguration::InstallAccessors(
        isolate, world, instance_template, prototype_template, interface_template,
        signature, kAccessorConfigurations,
