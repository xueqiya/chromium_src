...
class MODULES_EXPORT GPUTextureDescriptor : public GPUObjectDescriptorBase {
 public:
  ...
  bool hasSize() const { return !size_.IsNull(); }
  const UnsignedLongSequenceOrGPUExtent3DDict& size() const {
    return size_;
  }
  void setSize(const UnsignedLongSequenceOrGPUExtent3DDict&);
 ...
 private:
  ...
  UnsignedLongSequenceOrGPUExtent3DDict size_;
  uint32_t usage_;
