...
class MODULES_EXPORT GPUBindGroupLayoutBinding : public IDLDictionaryBase {
 public:
  ...
  bool hasTextureDimension() const { return !texture_dimension_.IsNull(); }
  const String& textureDimension() const {
    return texture_dimension_;
  }
  inline void setTextureDimension(const String&);
...
void GPUBindGroupLayoutBinding::setHasDynamicOffset(bool value) {
  has_dynamic_offset_ = value;
  has_has_dynamic_offset_ = true;
}

void GPUBindGroupLayoutBinding::setMultisampled(bool value) {
  multisampled_ = value;
  has_multisampled_ = true;
}

void GPUBindGroupLayoutBinding::setTextureComponentType(const String& value) {
  texture_component_type_ = value;
}

void GPUBindGroupLayoutBinding::setTextureDimension(const String& value) {
  texture_dimension_ = value;
}
