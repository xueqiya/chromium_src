...
class MODULES_EXPORT RTCRtpEncodingParameters : public RTCRtpCodingParameters {
 public:
  ...
  double maxFramerate() const {
    DCHECK(has_max_framerate_);
    return max_framerate_;
  }
  inline void setMaxFramerate(double);
  ...
  bool hasScalabilityMode() const { return !scalability_mode_.IsNull(); }
  const String& scalabilityMode() const {
    return scalability_mode_;
  }
  inline void setScalabilityMode(const String&);
...
void RTCRtpEncodingParameters::setMaxFramerate(double value) {
  max_framerate_ = value;
