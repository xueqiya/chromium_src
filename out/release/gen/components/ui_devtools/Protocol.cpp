...
using crdtp::cbor::InitialByteForEnvelope;
}  // namespace cbor
...
DispatcherBase::Callback::Callback(std::unique_ptr<DispatcherBase::WeakPtr> backendImpl, int callId, const String& method, crdtp::span<uint8_t> message)
...
void DispatcherBase::Callback::fallThroughIfActive()
{
    if (!m_backendImpl || !m_backendImpl->get())
        return;
    m_backendImpl->get()->channel()->fallThrough(m_callId, m_method, crdtp::SpanFrom(m_message));
    m_backendImpl = nullptr;
}
...
void UberDispatcher::dispatch(int callId, const String& in_method, std::unique_ptr<Value> parsedMessage, crdtp::span<uint8_t> rawMessage)
{
    String method = in_method;
    auto redirectIt = m_redirects.find(method);
    if (redirectIt != m_redirects.end())
...
{
    using crdtp::cbor::NewCBOREncoder;
